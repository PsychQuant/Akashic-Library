    ...
    static let tools: [Tool] = [
        ...
        Tool(name: "akashic_resolve_people",
             ...
             inputSchema: obj([
                ...
                "refute": strArray("逐篇**否決**（#386 的鏡像）：citekey:authorIndex:personKey=否決理由。與既有 reject 的差別是**歧義列也適用**（reject 只吃 resolver 提名出來的候選）。entry 不動，只寫 resolution-rejected verdict；之後該配對不再被提名。理由必填。失敗語意同 judge（store 狀態不符含 citekey 重複或與另一筆 work 共用 id，該筆略過並具名，#627）。對共用 literal 來說「不是他」通常才是答案"),
