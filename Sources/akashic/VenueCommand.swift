...
struct MigrateWorkTypes: ParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "migrate-work-types",
        abstract: "把 Entry.type 的自由字串遷到 APA7 ch10 的值域（#325 階段一；type 仍是 String）")
    ...
    func run() throws {
        let store = try options.openStore()
        // 同 migrate-venues／migrate-person-identity：破壞性批次遷移**先回顯目標**
        // ——LibraryLocator 對 CWD 零感知（registry current 決定目標）。
        print("目標 store：\(displaySafe(store.root.path, max: 300))")
        let report = try WorkTypeMigration.run(store: store, apply: apply)
        let prefix = apply ? "✓" : "（dry-run）"
        print("\(prefix) \(apply ? "已遷移" : "將遷移") \(report.planned.count) 筆"
              + "，已是新值域（跳過）\(report.alreadyMigrated.count) 筆")
        // 依舊值分組回顯，讓「三個舊值合流到 conference-session」看得見
        var byPair: [String: Int] = [:]
        for p in report.planned { byPair["\(p.from) → \(p.to)", default: 0] += 1 }
        for (pair, n) in byPair.sorted(by: { $0.value > $1.value }) {
            print("  \(displaySafe(pair, max: 120))  \(n) 筆")   // display-safe-exempt: n 是 Int
        }
        if !report.unmapped.isEmpty {
            print("對映不到 \(report.unmapped.count) 筆（**未遷移**，不猜）：")
            for f in report.unmapped.prefix(20) {
                print("  ⚠ \(displaySafe(f.citekey, max: 200))——\(displaySafe(f.reason, max: 300))")
            }
        }
        if !report.failed.isEmpty {
        ...
        if let next = WorkTypeMigration.nextStep(report: report, apply: apply) {
            print(next)
        }
    }
}
