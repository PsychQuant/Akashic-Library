...
- **承重頁面存檔——判定的證據今天做不到**：已判定配對的 digest 沒有地方寫（venue 的 `references` 沒有通用寫入面，#587）；**查過未決的配對可以**——digest 跟著未決記錄的 `rests_on` 走（#619）；WebFetch 回的是模型改寫稿、不是原始位元組（#591）。判定（confirmed／rejected）刻意不攜 rests-on（#280 裁決，同 person 域）；**未決記錄可以帶**——查過未決的配對，證據唯一的落點是那筆未決記錄（#619）
