...
///
/// **用於配對，永不用於判定。** 判定用的對偶是 `NameIdentity`（#296）——它零假陽性、
/// 只收空白變換。拿本函式去判定會**丟掉真正不同的名字**且無 verdict 可回溯；拿
/// `NameIdentity` 去配對會**漏掉該提名的候選**。兩者刻意分檔，各自的檔頭互相指認。
///
/// `matchingKey` 的輸出是比對鍵，不是資料：同 key
