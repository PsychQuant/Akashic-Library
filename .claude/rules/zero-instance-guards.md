...
**第 31 列的量測（2026-09-25，可重跑）**：warning 數 `grep -a -q 'person／organization 的 verdict 數逼近' "$(command -v akashic)" && akashic validate 2>&1 | grep -c 'person／organization 的 verdict 數逼近'`（應印 0；沒印任何東西＝舊 binary，同第 13 列的自證）。Python 對照（不依賴 binary；讀不到的檔計數，同第 24 列）：
...
for f in glob.glob(os.path.expanduser('~/.akashic/entities') + '/*.yaml'):
    ...
    for k in best:
        if k in d:
            n[k] += 1
            c = sum(1 for r in (d.get('references') or []) if isinstance(r, dict) and str(r.get('field', '')).startswith('resolution-'))
            if c > best[k][0]: best[k] = (c, str(d.get('key')))
print(n, '單筆最多', best, '讀不到的檔', bad, '門檻 11111')
EOF
# 2026-09-25：person 4575、organization 13｜單筆最多 person 40（yi-hau-chen）、organization 1｜讀不到 0
```
