...
| **同一段落裡連鎖六次**（#407 R21d–R25c）| 上一列是單次；這一列是它**連續發生六輪**，而且每一輪都在修上一輪。全部不是程式碼錯誤，全部是**描述超出證據**：「收窄是根治」（實為收窄＋一次性手動遮蔽）→「攤開表讓誤宣告可見」（印出來與正常依賴逐字相同）→「19 行單行 `run:`」（19 是總數、單行 13；`13+6+1=20≠19`）→「`case` 不能綁」（實測 3 格綁得起來）→「兩種都有負控背書」（只有一種）→ 而**那句更正本身**又只寫結論不寫查核（「`rule-coverage.sh` 不讀 `Sources/`」為真，但 diff 裡沒有 `grep -c 'Sources' … → 0`）。**位置最傷**：第五次出現在「剛刪掉一個稻草人」的同一段，第六次出現在「修正第五次」的同一段。六次全部由跨模型審查抓出，而且**都在該段話 commit 之後**——`git log --all --oneline --grep='#407 R21d' --grep='#407 R22b' --grep='#407 R23e' --grep='#407 R24g' --grep='#407 R25b' --grep='#407 R25d'`（多個 `--grep` 是 OR）回 `1b723e5`／`59ee59c`／`41bd3d8`／`848939a`／`08c15b0`／`3eda486`，六個都是**更正** commit。**git 只證立時序**：它證立不了「有一道檢查」，也證立不了「那道檢查通過了」——能誠實說的只有**commit 當下沒有被抓到**。
