...
- **第七次：這一輪改用另一種修法——把斷言換成可查核的形式**（#407 R25e，c18 的 DA 席指名）：R25c 那一列的**結尾金句**是「六次全部由跨模型審查抓出——**作者自審每一輪都認為那段話已經夠精確**」。後半句對**心理狀態**下斷言，無從查核；我沒有自審記錄，那是推論。**而它偏偏寫在描述前六次錯誤的那一列裡**。
  改成 git 查得到的事實：六次全部由跨模型審查抓出，且**都在該段話 commit 之後**——實際 hash 為 `1b723e5`／`59ee59c`／`41bd3d8`／`848939a`／`08c15b0`／`3eda486`，六個都是**更正** commit。**git 只證立時序**，證立不了「有一道檢查」或「那道檢查通過了」；能誠實說的只有**commit 當下沒有被抓到**。
