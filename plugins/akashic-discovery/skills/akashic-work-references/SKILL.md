...
- 讀 `warnings`，每一則都要處理。warning 裡的位置寫成「第 L 行（PDF 第 P 頁）」：告訴使用者時給 **PDF 頁碼**，那是他對照得到的。行號是 `paper.txt` 的座標：不要給使用者，也**不要拿去讀 `paper.txt`**（見上方「不要直接讀」）。
  ...
  - 最後一筆的三則 warning——「清單停在…，而最後一筆（第 k 筆）看起來還沒結束」、「清單一直到文字結尾，而最後一筆（第 k 筆）看起來還沒結束」、「…的下一行以小寫開頭——這個標題可能是最後一筆（第 k 筆）換行換出來的一個字」：第 k 筆可能被切斷，也可能吸進了清單之後的文字（本機語料裡「文字結尾」那一則 18 份觸發，看得出來的都是吸進了頁尾或收稿日期）。第 k 筆照常判定，但**比照「原文超過 1000 字」處理**：`doi` 不算證據；標題或出處看起來缺了一段就判為「判不了」。把 PDF 頁碼告訴使用者（「文字結尾」那一則沒有頁碼）（R6、R7）。
  - 「沒有辨識出年份」：可能是殘留的段落文字。
