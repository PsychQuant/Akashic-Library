...
#     · 宣告指向守衛自己所在的目錄（可能多餘、也可能必要）
#     · 宣告的目錄名在原始碼裡沒有明顯痕跡（字串比對判不出「在不在讀」）
#
# **核心職責永遠在 fails 這一側。** 降級的兩條都是「這條宣告是否屬實」的
# 判斷——而那是靜態分析問題，字串比對做不到（三版近似兩頭都不對，見下方
# 痕跡檢查的註解）。降級的不是嚴重度，是**我們有沒有能力判定**。
#
...
for g in GUARDS:
    ...
    # **近似宣告**（#407 R27）：整行就是 `trigger-coverage: reads X` 卻**少了註解
    # 標記**，於是 DECLARE 不匹配、宣告完全啞掉——而輸出與「沒有依賴」逐字相同。
    # 實地踩到：`decision-matrix-drift.py` 的宣告寫在 docstring 裡沒加 `#`，
    ...
    for line in decl_lines:
        ...
        # ——**一種**宣告為真的寫法會被誤殺，它有出貨的負控直接背書：
        #
        #   for f in "$BASE"/*/*.md      純 glob，從不寫目錄名
        #     負控：`守衛用純 glob 讀宣告的目錄、從不寫目錄名（宣告為真，不得誤殺）`
        #
        # 原本這裡列了第二個（`Sources/*/*.swift`，中間萬用字元）並宣稱它也有
        # 負控背書。**那句不準確**（#407 R25b，DA 席指名）：那一格的注入是把
        # `rule-coverage.sh` 的宣告改成 `Sources/*/*.swift`，而它不讀 `Sources/`
        # ——那是**編造的宣告**，測的是「中間萬用字元讓痕跡檢查走回一個不在
        # 原始碼裡的段」（R22e 的性質），不是「宣告為真卻被誤殺」。
        #
        #     量測（#407 R25c 補——上一版只寫結論不寫查核步驟）：
        #       grep -c 'Sources' plugin/tests/rule-coverage.sh   → 0
        #     它讀的是 "$RULES"/*.md 與 "$SKILLS"/*/（第 35、38 行），與
        #     `Sources/` 無關。
        #
        # 論證不受影響——**而「不受影響」這句也該有依據**：R24f 的裁決結構是
        # 「誤殺傷害目標使用者 vs 漏放的真實實例為 0」，那是一個**代價比較**，
        # 不是例子計數；**一個**成立的誤殺例子就讓左側非空，比較即可進行。
        # （R24f 的原文在本檔上方——`grep -n 'for f in "\$BASE"' ` 指到**第 581 行**，
        #   而本行約在 598，確實在其下方。上一版只寫「本檔上方」沒給行號，
        #   而同段落緊鄰的兩處都給了可重跑的查核——#407 R25e，DA 席指名。）
        #
        # 「兩種都有背書」是一句沒被檢查的話，而它出現在**剛刪掉一個稻草人的
        # 同一段**裡——而它的更正（R25b）又只寫結論不寫查核，被 DA 席再抓一次。
        # **同型第六次**，記在這裡當作那條規則的實例。
        #
        # **原本還列了第三個**（`D=$(dirname "$0")/../rul` ＋ `D="${D}es"`，把
        # 目錄名從中間切開再拼接）。跨模型審查指出它是**稻草人**——全 repo 搜
        # 不到任何守衛用這種寫法（實測 0 命中），它是為了「無論字面比對多寬鬆
        # 都抓不到」而構造的。刪掉：三個例證裡混一個未經檢驗的，會讓整段論證
        # 打折，而這段論證正是用來證成一次 revert 的（#407 R25，DA 席指名）。
        #
        # **第一個正是宣告機制存在的理由**：宣告是為了補「守衛用 glob 組路徑、
        ...
        # **兩種證據狀態保留各自的文字，但同屬 warning。**（#407 R25）
        #
        # R24f 退回兩級時把兩者併成同一則訊息，於是「seg 一次都沒出現」與
        # 「seg 出現但不在路徑脈絡」對讀輸出的人**長得一樣**——而前者是強得多
        # 的編造訊號。跨模型審查指出這讓人無法排優先序。
        #
        # 保留區別但**不重新分層**：分層會把誤殺帶回來（R24f 的裁決），而
        # 保留文字只是把已經算出來的訊息強度說出來。
        if seg and '*' not in seg and '?' not in seg and not traced:
            if seg not in body:
                warnings.append(f'{os.path.basename(g)} 宣告讀 `{glob_}`，而 `{seg}` '
                                f'在它的原始碼裡（扣掉宣告行本身）**一次都沒出現過**'
                                f'——編造訊號最強的一種。但守衛也可能用純 glob／'
                                f'動態路徑讀它（那正是宣告要補的漏），所以仍是'
                                f'警告而非缺口（理由見上方 R24f 的裁決）。請人確認')
            else:
                warnings.append(f'{os.path.basename(g)} 宣告讀 `{glob_}`，`{seg}` '
                                f'有出現但不在路徑脈絡裡——**訊號較弱**（可能是散文'
                                f'巧合，也可能是裸目錄名的真實用法）。請人確認')
    # **這一條目前不可獨立觸發，保留是有條件的。**
    #
