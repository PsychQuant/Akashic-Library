...
- **把連鎖形狀寫進規則自己的失敗表**（#407 R25c）：`assertions-must-be-measured.md` 的「這份文件犯過的錯」已有 13 列，最後一列（「修掉假話時產生新的假話」）是**單次**的雛形。新增一列記**連續六輪**：「收窄是根治」→「攤開表讓誤宣告可見」→「19 行單行 `run:`」（`13+6+1=20≠19`）→「`case` 不能綁」→「兩種都有負控背書」→「更正本身不寫查核」。**全部不是程式碼錯誤，全部是描述超出證據**；六次全部由跨模型審查抓出，而**作者自審每一輪都認為那段話已經夠精確**。
