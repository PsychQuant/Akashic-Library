...
- 對 store 跑 `akashic export-bib` 並與升格前的輸出逐位元比對：`DOI`／`ISBN`／`PMID`
  **除下述 15 筆之外**無差異（那 15 筆須逐筆出現在遷移報告裡）；`ISSN` **零損失**，
  且新增的筆數與「該 venue 有 ISSN 而 work 先前沒帶」的數量相符。

  > **`ISSN` 這一條是 2026-08-24 遷移後補的，而它補的是一個驗收自己的盲點。**
  > 原條件只列 `DOI`／`ISBN`／`PMID` 三個欄位——寫驗收的人與寫 §7 mitigation 的人
  > 在**同一個盲點**裡：那三個遷移後仍在 work 上，而 `issn` **換了實體**（搬到 venue），
  > 所以「在自由字典迴圈之後寫結構化值」這個形狀對它套不上去。
  >
  > 實測後果：遷移後 `.bib` 的 ISSN 欄位自 64 掉到 **0**——資料沒丟（在 venue 上），
  > 但匯出看不到，**而且不會報錯**。這正是 design 自己在 Risks 段預言的
  > 「export 靜默少欄位」，只是它預言的是 `doi`、實際發生在 `issn`。
  >
  > 修法是讓 `BibExport` 從 work 的 venue 邊撈 ISSN。修好後實測：**失去 0 筆**、
  > 值被正規化 41 筆（`00063444, 14643510` → `0006-3444, 1464-3510`）、
  > **新增 114 筆**——刊登在那 39 個 venue 但先前沒帶字串的 work。使用者裁定保留：
  > 那 114 筆正是「識別碼住在它所識別的實體上」在匯出面顯現的效果。
  >
