...
| 19 行 `run:` 的分類 | 「19 行**單行** `run:`」 | 19 是總數，單行 13（且 `13+6+1=20≠19`）|
...
| **查核本身用錯量測，而那次更正也是錯的**（#407 R25c／R25i）| 為了避免第七次而逐項查核時，用 `grep -c '一次都沒出現過'` 回 3，把它讀成「3 格用這個 expect」，然後**推翻自己**寫下「真正用它當 expect 的只有 1 格」。**那個 1 也沒查**——逐格列出（`3eda486^` 當時與現在皆然）：**3 格全是 `warn_case`**，`grep -c` 的 3 碰巧就是格數。**推翻一個沒查的數字時，我給了另一個沒查的數字。** 這一列與下一列的差別是層次——下一列是斷言沒被量測，這一列是**量測與它的更正都沒被檢查** |
| **同一段落裡連鎖六次**（#407 R21d–R25c）| 上一列是單次；這一列是它**連續發生六輪**，而且每一輪都在修上一輪。全部不是程式碼錯誤，全部是**描述超出證據**：「收窄是根治」（實為收窄＋一次性手動遮蔽）→「攤開表讓誤宣告可見」（印出來與正常依賴逐字相同）→「19 行單行 `run:`」（19 是總數、單行 13；`13+6+1=20≠19`）→「`case` 不能綁」（實測 3 格綁得起來）→「兩種都有負控背書」（只有一種）→ 而**那句更正本身**又只寫結論不寫查核（「`rule-coverage.sh` 不讀 `Sources/`」為真，但 diff 裡沒有 `grep -c 'Sources' … → 0`）。**位置最傷**：第五次出現在「剛刪掉一個稻草人」的同一段，第六次出現在「修正第五次」的同一段。六次全部由跨模型審查抓出。**能機械查出時序的有三對**（引入該句的 commit → 修正它的 commit）：`41bd3d8 → 9eb9737`（「19 行單行 `run:`」）、`848939a → f5a3dc9`（「`case` 不能綁」）、`08c15b0 → c392302`（「更正不寫查核」）。**其餘三次查不出**——它們的引入與修正落在同一個 commit 裡（那一輪同時寫下並改掉），或編號只出現一次。
  上一版寫「六個都是**更正** commit」——**那個標籤沒被查核**（#407 R25h，DA 席指名第九次）。實測：`41bd3d8`／`848939a`／`3eda486` 是**純新增**（`git show --numstat` 刪除行數 0），分別是**引入**與**記錄**，不是更正。用一個真的查詢，支撐一個那個查詢沒問的性質——與第八次同型。
  **git 只證立時序**：它證立不了「有一道檢查」，也證立不了「那道檢查通過了」——能誠實說的只有**commit 當下沒有被抓到**。
  這一列的結尾金句被改過**兩次**：第一版寫「作者自審每一輪都認為那段話已經夠精確」（心理狀態，無從查核，#407 R25e）；第二版寫「也就是說每一次它都通過了作者當下的檢查才進版控」——**用「也就是說」把時序擴張成認知宣稱**，而它與這一列要講的事自相矛盾（若真通過了檢查，六次連鎖就不該發生）。第八次，DA 席指名（#407 R25g） |
