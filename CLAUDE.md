...
> | 最大一支 | `marker-parity-mutations.py` 52.5 秒（88%） | 同一支 **57.2 秒**（佔守衛的 86%） |
...
> 判準，這次踩在它自己記過的形狀上）。
>
> **後果是裁決的方向也要改**：把那支守衛移出 CI，hook 從 **252 → 195 秒（−23%）**，
...
>
> **現況是第 2 列**（正常 push × CI 不跑 × hooksPath 指向主 repo）；各 hooksPath 值的
> 意義見下方三點。
>
...
>
> 所以「移出」換到的是速度（實測 252 → 195 秒，−23%；**不是**上一版寫的
