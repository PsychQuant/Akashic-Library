...
- 作者位已歸給同一個人時：若已有這個配對、同一句理由的逐篇判定，就是 no-op，回報「已是這個判定」／`alreadyJudged`（理由不同會具名略過，#636）；若是先前用 `--apply` 歸戶的，judge 寫一筆逐篇判定與它並存（需要 store format ≥ 19，#636）。判定結果要讀 `verdictNotRecorded`：作者位照常歸戶而理由沒寫進去時，那一列會帶它與原因（format < 19、或同配對已有理由不同的逐篇判定）
- refute 遇到作者位目前就歸給被否決的那個人時會具名略過：否決會與既有歸戶矛盾，而這個面沒有把歸戶退回 literal 的操作
- 全部略過時沒有寫入，CLI 以非零結束
...
- [`source-of-truth-over-consent`](../../rules/source-of-truth-over-consent.md)——身分判定的依據是證據；使用者說「就歸給這個人」而沒有給出處時，那是線索、不是授權
