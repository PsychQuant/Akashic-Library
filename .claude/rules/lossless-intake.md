...
## 有界拒絕：整批擋下並出聲，**不是**上面那張表的第三類（#519，2026-09-08）

`AddOnlyEnrichment` 對單一字串設了 **65,536 位元組**的上限（`maxValueBytes`）。超限時**整批
拒絕、零寫入、具名**——不是丟掉那個欄位、收下其餘。

**這一條刻意寫在封閉列舉之外，而不是當成第 3 類。** 理由是兩者的失敗形狀相反：

| | 上面那張表管的 | 本節管的 |
|---|---|---|
| 結果 | 收了一部分、丟了一部分 | **一筆都沒收** |
| 使用者知道嗎 | 若不報就不知道（本規則的核心禁令） | **必然知道**——那是錯誤，不是報告裡的一列 |
| 事後可判定嗎 | 不可（來源已不在） | **可**——來源原封不動，改完再跑一次 |

把它列成第 3 類會產生一個真的危險：那張表的成員資格是「**可以丟掉這個欄位、繼續匯入**」，
而一個以「大小」為名的成員會被下一個人**依性質相似類推**成「太長的欄位可以丟掉」——那正是
本規則第 3 條執行細節禁止的靜默丟棄。**兩類仍然是兩類，不得類推第三類。**

**為什麼可以有這一節**：本規則的不對稱論證是「沒收 → 那件事永遠不可判定，**而且沒有任何
跡象**」。有界拒絕兩半都不成立——來源還在，而且有一條具名的錯誤。所以它不在本規則要防的
行為之內；寫下來是為了讓下一個人不必重新推導，也為了讓「為什麼不是第 3 類」有出處。

**邊界條件（缺一即回到本規則的禁令之內）**：

1. **整批**，不是逐筆略過——部分寫入會讓「哪些進去了」需要讀報告才知道。
2. **零寫入**，不是先寫再回滾。
3. **具名**：說出是哪一筆、哪個鍵、實際多長、上限多少。
4. **不截斷**。截斷會讓一個**不是來源給的**值進 store，那是有損且靜默，直接違反本規則。
5. **上限的值要有量測出處**，不是憑感覺挑的數字（`assertions-must-be-measured`）。

裁決與量測見 `zero-instance-guards` 的對應列與 #519。
