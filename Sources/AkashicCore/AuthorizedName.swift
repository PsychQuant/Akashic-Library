...
public enum NameForm {
    ...
    /// `Author Full Names` 欄與 Crossref 作者欄對名字做的變換。實測（2026-08-10）全
    /// store 1147 個 person names 條目中 872 筆（**76.0%**）是這個形態，其中僅 38 筆
    /// （占引用形 4.4%）是 `姓, 縮寫`——**主體是完整的 `姓, 名`**。
    ///
    /// > 先前這裡寫 1144／76.2%：分子相同、分母不同，那是**只更新了一半**的指紋
    /// > （#222 verify）。重新量測時兩個數字要一起量。
    ///
