...
- 結束碼為 0，但 `refs.json` 沒有 `contract` 或小於 2 → 見「開始前」第 1 點，停。
- 讀 `warnings`，每一則都要處理。warning 裡的位置寫成「第 L 行（PDF 第 P 頁）」：告訴使用者時給 **PDF 頁碼**，那是他對照得到的；行號是 `paper.txt` 的座標，只供你自己對照 `raw`。
  ...
  - **「清單停在…的結束標題，但它之後還有 N 個條目開頭沒有計入」**：**停下來**，把 PDF 頁碼與數目告訴使用者，請他確認清單是否在那裡結束。在他回覆之前不寫入。
  - 「…夾在清單中間：清單在…接續」：清單沒有被截斷；那一段之前的最後一筆若有一段落在略過的區域裡，會缺那一段。那一筆照常判定，標題或出處看起來缺了就判為「判不了」。
  - 「參考文獻標題在清單中又出現 N 次…視為頁首」：清單跨頁、每頁印著同一個標題，已併成一份。不需要處理。
  - 「第 N 筆吸收了一行看起來像新條目開頭的內容」：前一筆的年份寫法沒認出來，兩筆被併成一筆，這筆的作者與標題可能分屬兩篇。判定時把它當成「判不了」，除非你能從 `raw` 看出是哪兩筆。
  - 「沒有辨識出年份」：可能是殘留的段落文字。
  - 「略過 N 行…落在第 x 筆」：被略過的是頁碼，與頁首頁尾、版權聲明這類重複行。列出的那幾筆若標題或出處看起來缺了一段，判為「判不了」。
...
同樣先看結束碼（非零 → 讀 stderr，多半是某個 `--openalex` 檔不是 OpenAlex 的回應，停），再看 `contract`（沒有或小於 2 → 見「開始前」第 1 點，停）。
...
（第 1、2 項不成立的那一筆，照第 4 步改判「已在庫只連」或「判不了」，不影響其他筆。）
...
- library（D5）：**一律不自動掛**。用 `akashic_libraries(action: "list")` 讀出種子所屬每個 library 的名稱與描述，在乾跑裡列給使用者，由他逐一指定要掛哪個 library、掛哪幾筆（每次執行問一次）；種子屬於 0 個 → 不掛、不問。使用者指定之後，照 [`source-of-truth-over-consent`](../../rules/source-of-truth-over-consent.md) 核對：
  - 描述表明成員由規則決定的（某期刊的全量、某 venue 的 works）→ 只掛符合那條規則的；不符的不掛，乾跑逐筆寫出「要求／規則／實際寫入」。
  - 描述表明成員是某一份文件的文獻（一門課的引用文獻、一篇寫作中論文的文獻）→ 只掛那份文件自己也引的；那份文件不在庫、查不到它引了什麼 → 不掛，列進報告請使用者確認。
  - 其他（成員由人依主題挑選）→ 使用者的選擇就是依據，照他指定的掛。
...
- **切分是啟發式的**：斷點（頁首、圖表、補充資料或致謝等結束標題）之後是否接續，是用「作者按字母順序接得上、年份括號後接標題」判定的。作者—年份格式以外的清單（例如 `[AB99]` 標籤制書目、年份不帶括號的書目）、條目沒有按作者排序的清單、嚴重的排版會切錯。`warnings` 與「只在 PDF／只在 OpenAlex」兩類就是讓這些誤差被看見的地方。R3 用本機 169 份真實 PDF 量過（只記數字）：切得出的 124 份，條目數低於估計值 80% 的有 3 份，都有多段或截斷的 warning。
