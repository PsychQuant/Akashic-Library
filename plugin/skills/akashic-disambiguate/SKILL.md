...
- 作者位已歸給同一個人時：若已有這個配對、同一句理由的逐篇判定，就是 no-op，回報「已是這個判定」／`alreadyJudged`（理由不同會具名略過，#636）；若是先前用 `--apply` 歸戶的，judge 會具名略過：verdict 以配對去重，理由無處另存（#636）。所以要留證據，就在 apply **之前**用 judge
