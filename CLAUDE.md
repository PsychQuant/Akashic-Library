...
> （#407 R26j，自己窮舉時找到）：
>
> | push 方式 | CI 狀態 | 留在 pre-push | 移出、只留 CI |
> |---|---|---|---|
> | 正常 `git push` | macOS 不跑（**現況**） | **執行** | **零執行** |
> | 正常 `git push` | macOS 恢復 | 執行 | 執行 |
> | `--no-verify` | macOS 不跑（**現況**） | 零執行 | 零執行 |
> | `--no-verify` | macOS 恢復 | 執行 | 執行 |
>
> **現況（上半兩列的「不跑」那兩格）：留著確實提供預設保護**——正常 push 時兩者的差別
> 正是「有執行 vs 零執行」。R26f 曾寫「差別只是摩擦力」，那句話只在 `--no-verify` 那列
> 成立而被推廣到全部情境（R26h 已修）。
>
> **而 macOS 恢復後，兩個選項在全部四種情境下等價**——那時「移出」是純效益：pre-push 從
> 59 秒降到 7 秒，而 7 秒的 hook 沒人想繞過。**這一列正是決定的關鍵，而三列版看不到它**，
> 因為它把「CI 恢復」寫成單一列，掩蓋了那個條件**對兩個維度同時起作用**。
>
