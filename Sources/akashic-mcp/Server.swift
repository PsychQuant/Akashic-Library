...
"split_author": strArray("把一個作者位**拆成多個**（#443）：citekey:authorIndex:分隔符=理由。一個 literal 裝了兩個人時用它。**收分隔符不收拆好的名字**——後者等於讓呼叫端編造；收分隔符則拆出的每一段必然是原文的子字串。切出空段即拒絕；只作用於未歸戶的位置；拆出來的仍是 .literal（拆是形狀修正不是身分判定）。單獨呼叫，不與其餘腿組合——它改的是作者位的數量"),
        ...
        Tool(name: "akashic_add_venue",
             ...
             inputSchema: obj([
                ...
                "names": strArray("名稱變體（正式刊名、縮寫、WoS 大寫形）。契約同 update_venue 的 add_names（#554 D8）：以 canonical 形入庫（空白類——含 tab／換行／LS——收斂為單一空格、NFC）、近重複只留一筆、空白項略過；含其他控制／格式／不可見字元、拉丁或 CJK 之間的接合字元、或沒有任何字母或數字的字串**整個呼叫拒絕、零寫入**；全部空白＝沒有名字，同樣拒絕。回報：`names` 是存入的拼法；送進來的拼法若被折成 canonical 形才存（尾隨空白、NFD）列在 `namesRewritten`，真的沒進 store 的（整項空白）列在 `namesDropped`"),
        ...
        Tool(name: "akashic_update_venue",
             ...
             inputSchema: obj([
                ...
                "add_names": strArray("要附加的名稱變體。相等看 canonical（前後／連續空白、NFC）——近重複自動略過、以 namesAdded 回報實際寫入的拼法，送進來的拼法若被折成 canonical 形（尾隨空白、NFD——canonical 形剛存入或本來就在）列在 namesRewritten、真的沒進 store 的（整項空白）列在 namesDropped；新名字以 canonical 形入庫：**空白類**（含 tab／換行／LS／PS／NEL）收斂為單一空格、NFC。**其他**控制／格式／不可見字元（bidi、零寬、變體選擇子、填充字元）、拉丁或 CJK 之間的接合字元（ZWJ／ZWNJ 只在使用 join control 的文字——阿拉伯系／印度系／蒙古文等——裡合法：virama 之後、左鄰是同文字的字母／標記／數字而右鄰是同文字的字母／數字（右鄰不收標記）、或 Devanagari／Bengali 的 virama 之前且其後接字母）、或沒有任何字母或數字的字串**整批拒絕零寫入**（同一呼叫的其他參數也不寫，#554 D8）"),
                "note": str("備註（替換；選填）"),
