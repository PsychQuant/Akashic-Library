...
| 寫入面 | 種類 | 一句話理由 |
|---|---|---|
| `resolve-people apply`／`reject`／`judge`（#272／#303／#386） | AI | `literal → key` 是身分判定；Jaccard 實測同一人 0.40、不同人 0.50，字串謂詞站在錯的一側。**change `resolution-verdict-states`（#636）**：同一配對的提名層判定（apply／reject）與逐篇判定（judge／refute）**兩筆並存**——判定層級（nominated／judged，封閉二值、由 rule 導出）參與記錄鍵；在此之前逐篇判定被當成重複丟掉，查證理由無處可寫 |
...
| `resolve-venues apply`／`reject`／`repoint`／`demote`（#304／#418） | AI | 同 people；`demote` 是判定的逆轉，原字串從 verdict 逐字取回、取不到寧可拒絕；≥2 個不同 literal 時同樣拒絕不判定（#554 R9，D23）。**翻轉判定時退役相反的那筆**（#554 R9，D20）：`repoint`／`demote` 寫 rejected 時把同 holder 上同一配對的 confirmed 移除（反向 repoint 對 rejected 亦然）——verdict 沒有時間戳，「後者為準」只有寫入面知道，留著就是 #486 的矛盾對；歷史留 git（同 un-split 的取捨）。**前提是配對只由一條邊實例化**（#554 R10，D25）：verdict 不帶 venue index，同一 work 兩條邊指同一 venue時退役會把另一條邊的證據一起刪——那種 work 具名拒絕零寫入；退役的每筆逐字回報（截 20 筆、總數揭露，R11）。**唯一性對寫入後的邊集合驗，生產端也擋**（#554 R11，D27／D28）：R10 的 D25 只看原始 entry 的 from——`repoint` 改指到本 work 已有邊的 venue、`apply` 對兩條同刊名的 literal 邊各 apply 一次，都造得出 D25 宣告不得存在的形；現在 `apply`／`repoint` 對寫入後會出現「同一 work 兩條 key 邊指同一 venue」的整批拒絕零寫入，同一批同一 work 的兩個 move 不得帶同一個 literal（逐 move 的退役會互相覆蓋）。既有的這種 work 由 `Entry.validate()` 報 warning；刪掉多餘邊的面另案 #572——落地前出路只有手改 YAML。**R12（D33）：`apply` 改逐筆略過並具名**（`skippedDuplicateVenueEdge`）——整批拒絕讓一筆毒候選殺掉同批無關的候選、既有的重複邊鎖死不相干的歸戶；store 狀態不符是「該筆略過並具名」（`judge` 的先例），語法錯才整批拒。D27 只看被動到的邊、同 literal 的 move 只在 venue 集合相交時拒；否決抑制改比 `matchingKey`（與其餘三處同粒度——代價寫在兩面描述：對一個拼法的 reject／demote 會壓住同 work 同 venue 的其他拼法，撤回面 #559，R14 verify regression 第 21 列）。**R15（D38）：目的 venue 已對該 work 持有另一個正規化後不同的 confirmed literal（沒有對應的邊）時，`apply` 逐筆略過並具名（`skippedConflictingConfirmedLiteral`）、`repoint` 對預測後的 verdict 集合驗、整批拒絕**——R14 verify Codex 第 1 列：生產端的 fail-closed 只在合併有，apply／repoint 寫下第二個 confirmed 就把那條邊鎖進 D23。**R16（D43／D44）：閘的相等比位元組、同一 literal 的另一個拼法也略過／拒**（R15 verify 第 3 列 HIGH、第 5 列：R15 以 `matchingKey` 比、放行同鍵異拼法——新拼法沒寫入，之後 demote 還回目的 venue 的舊拼法而不是這條邊的原字串，正是 `confirmedLiteral` 承諾不做的事；D23 比位元組，閘要同一把）；apply 的閘移到 D28／D33 的重複邊檢查**之後**（第 2 列 HIGH：重複來源欄位這個最常見情境被分進錯的桶、出路叫人刪掉有邊的 verdict）。**R17（D50）**：閘的訊息兩桶同時非空時兩桶都說、出路分開列（R16 verify DA 第 9 列：只印第一桶、用單數「那筆」，照著刪掉一筆之後 validate 全綠、再跑才撞第二桶） |
| `resolve-people undecided`／`resolve-venues undecided`／`resolve-organizations undecided`（#619；change `resolution-verdict-states`；org 族 #643，change `org-undecided-leg`） | AI | **「查過、判不出來」也是判定工作的產出**：寫它的人查了名字以外的證據而結論是不足以判定——那個結論與它依據的內容（rests-on）正是 `identity-is-judged-not-matched` 要求留下的東西，只是方向是「不下判定」。所以它走 AI 欄的義務：說明必填、記錄必留、可附證據 digest。**它不是判定的逆轉也不動 entry**：未決不抑制提名、不構成矛盾，配對被判定之後保留為查證歷史（不退役）。已判定的配對寫未決＝逐筆略過（未決不改變已判定配對的狀態）。落地是程式（整批拒絕、逐筆略過、完全相同＝no-op） |
| `resolve-organizations`（#304） | AI | 同上 |
| `resolve-divergence`（#71 一族）／攣生合併（#456／#459／#553） | AI | 兩筆記錄是否同一實體是判定；合併含全庫改寫＋刪檔，所以落地那一半是程式且要乾跑過目。**#553 起 venue 走同一個面**（不另開列——列是「寫入面」，而這是同一個面多收一個 shape）。venue 側有一格與 person 側**相反**且必須寫出來：被併者的 `authorized` 在 person 是**拒絕條件**（#81：「哪個名字對外」是判定），在 venue 只是**提醒**——實測 479/479 筆 venue 的 `authorized` 恰好等於 `[names[0]]`，唯一寫入者**當時**是 `VenueBootstrap` 的建檔慣例，所以它不承載判定。拿一個不做判定的操作的副產品當拒絕條件，會讓合併對它要解決的 7 組重複全部無用。**#554 起有面可寫（`update-venue --authorize`），這一格 2026-09-12 重開過一次、裁決不變**——理由換了：那個面不留 judgement（#564），所以「人確認過的 names[0]」與機械值在 store 裡仍然長得一樣，合併端拿不到可以承重的東西。#564 裁「留」且落地後升成拒絕條件（`DivergenceResolve.authorizedDemotedByMerging` 的 doc 記著觸發條件）。**#554 R12 兩條新拒絕（D31／D32，person 與 venue 同）**：倖存者與被併者對同一配對持相反判定時拒——合併不裁決哪一筆對，那是兩個判定的衝突（#486 的矛盾對，處置沒有工具面）；venue 合併把同一 work 的兩條 key 邊塌成一條而兩筆正規化後不同的 confirmed literal 都留下時拒（塌完之後 D23 讓那條邊永久不可 demote）。被併者的 verdict 遷移以 `verdictEqualityKey` 去重——R11 之前比位元組，「(field, value) 冪等」只在位元組層為真。**R13**：D31 累積比對（三方合併的 doomed↔doomed）且裝在 holder 遷移那一步（work／person 合併把第三方 holder 的 verdict 改寫成倖存者後不得矛盾——R12 verify DA 真 binary 全工具面重現）；D32 守不變式本身（這次新增的 confirmed literal 讓倖存者對某 work ≥2 個正規化後不同的 confirmed 即拒，不看塌不塌邊；倖存者既有的違反不擋）；去重丟掉的判定逐筆回報在 `verdictsCollapsed`。**R14（D34／D35）**：三處閘收成同一個 delta 謂詞——合併後與合併前比、只擋這次帶進來的（相反判定三種 shape；≥2 個 confirmed literal 只對 venue 記錄），合併前就有的違反不擋（validate 的 warning 負責——R13 的 holder 側是絕對謂詞、keeper 側漏單一被併者內部的矛盾與雙 literal）；holder 遷移那條路兩半都算（R13 只補了相反判定，work 合併純工具面造得出 D23 從此拒的邊）；訊息印出處與遷移前的原值；dry-run 與實跑對 `verdictsCollapsed` 同源。**R15（D37／D40）**：差集在**索引層**算——R14 以配對鍵比未改寫的 before 與改寫後的 after，被併鍵上的既有矛盾對與雙 literal 改寫後鍵變了、被判成新（R14 verify logic 第 2 列 HIGH，四席同指）；現在 after 的違反是既有的當且僅當構成它的索引裡有一組在合併前就構成同一類違反，含倖存者自己持有的 `person:<被併>` 矛盾對；雙 literal 的拒絕把帶進來的與倖存者既有的分開列；收攏丟列印遷移前的原值並逐段截（rename 同一個生產者）。**R16（D45）：「既有」看倖存配對**（R15 verify 第 12／13／20 列：R15 問「有沒有某一個合併前分區已持有全部」——holder 對 doom 持 {A, B}、對 keep 持 {A}，doom 那一區 ⊇ 全部就放行，而 (holder, keep) 合併前只有一個 literal、可以 demote，合併後被 D23 鎖住；末句「含住在被併鍵上的不擋」與判準互相矛盾）：不擋的只有倖存配對合併前就持有整組、或倖存配對合併前一筆都沒有而整組原樣從單一被併鍵搬來；整組住在被併記錄裡的（R13 的裁決不變）、或讓倖存配對的既有歧義變大的，都擋——矛盾對同一條規則。**R17（D47）：收攏的勝者政策**（R16 verify DA 第 1 列 HIGH，真 binary 重現：#468 的「弱血統優先」排在「未改寫者勝」之前，被併記錄弱血統的 `VEE JOURNAL` 贏過倖存者使用者確認的 `Vee Journal`、連位元組一起取代，之後 demote 把邊寫成不是它原本記的字）——碰撞的 literal **位元組不同**時倖存配對自己的（未改寫）那筆勝，同拼法時 #468 三層照舊（留弱只多一句警告、不動任何字串）；被丟的那筆連同留下的拼法印在 `verdictsCollapsed`（三條收攏路徑——holder 遷移、keeper 合併、rename——同一個描述函式）。這是對 #468 使用者裁決的邊界收窄，使用者可翻。**R18（D51／D53）：倖存配對不是倖存邊**（R17 verify Codex 第 1 列 HIGH：keeper 對某 work 持有的 confirmed 可能**沒有邊**——手改、舊 binary、D38 具名的那種輸入——而 work 的唯一邊指向被併 venue，被併記錄的那筆才是那條邊記錄的字；R17 的 keeper 路徑一律 keeper 勝、三方合併由陣列順序決定、rename 沒有任何政策）：三條收攏路徑同一個勝者函式（`collapseWinner`）——拼法位元組不同時先留**活著的邊**那一筆（合併前 work 對該 venue／person 真的有邊），再留與倖存配對自己位元組相同的，再 #468 三層，最後首見；rename 只收攏它動到的鍵，被改寫的那筆對上早已指向新鍵的死 verdict 時留活的，兩筆都沒動到的碰撞留著（validate 照報）；**R19（D55）**：以鍵整組算——R18 的單一槽位記帳讓三方碰撞由 YAML 順序決定（R18 verify Codex 第 1 列 HIGH），現在被改寫的全留、早已指向新鍵的全丟。**代價寫出來**（R17 verify regression 第 9 列）：D47／D51 留強丟弱時，弱血統那筆的 `rule` 尾註從 store 消失——它的消費端不只合併那一刻的收攏列，還有之後每一次提名（`PersonResolver`／`VenueResolver` 的 confirmed-elsewhere 從 `confirmedByLiteral` 讀 rule 揭露弱出身）；#468 保的那個警告在這一格自此只在 `verdictsCollapsed` 出現一次。使用者可翻的是這個取捨。**R20（D58；R19 verify DA 第 1 列 HIGH）**：D55 以含 field 的鍵分組，早已指向新鍵的 rejected（異 field）逃過「全丟」、rename 後復活成 #486 矛盾對——D58 改成不論 field 與拼法一律丟；被改寫的同拼法重複走 `collapseWinner`（#468 那一層在 rename 之前沒跑，R19 留首見）；拼法不同的都留——venue×work 以外的六格沒有掃描面，rename 不替它們判定（§3.5 誠實邊界）。**R21（D60；R20 verify 五席）**：D58 的「一律丟」寫在第一段後的早退之後，holder 只持有死 verdict 時整筆記錄原樣通過、rename 後復活（四席同指，真 binary）；而 DA 指出即使修好，那也是程式編輯**單方面銷毀判定編輯的產物**——無乾跑、無逆操作、無 git 閘，被丟的可能是人對另一筆仍存在的記錄親自下的判定。現在與 merge 的 D31／D34 同向：目的鍵上已有 verdict（同 holderKind、不論 field 與拼法）即 rename 具名拒絕、零寫入（`assertNoVerdictAlreadyAt`，逐筆列 holder／欄位／value／rests-on，出路是改那一行的 value 或刪掉它）。**R22（D61／D62；R21 verify）**：母體含 quarantined 檔（行級比對，DA 真 binary：R21 讓一筆 validate 本來會報的死 verdict 在 rename 後變成沒有任何面會報的活斷言）；折疊只折完全相同的重複（R21 verify DA 第 14 列：同拼法而 judgement／rests-on 不同的曾被折成一筆，digest 永久消失）——「rename 不做任何判定的刪除」自此才是真話。**R23（D63／D64；R22 verify，6 席齊）**：行級比對被 YAML 折行擊穿（needle 含空白，四席同指、live store 2 筆）→ 位元組比對 `<kind>:<newKey>`，與本列 merge 隔離檔閘同一種紀律與同一種誠實措辭；D62 留下的同鍵異 judgement 有了掃描面（`duplicateVerdictRecords`），merge 收攏它時在 `verdictsCollapsed` 回報、`describeDedupedVerdict` 自此也印 rests-on（Codex 第 6 列：R20 只補了 `describeCollapsedVerdict`）。**R25（D69；R24 verify 第 9／26／29 列、DA 第 39 列）**：`fieldsLostByMerging` 的「倖存者身上已經有這筆 reference」改比 `byteExactKey`——canonical `==` 曾把被併記錄的 NFD 拼法判成「已有」、那組位元組隨檔案消失而報告說沒遺失，是 D65 在合併面的同一個缺陷；`verdictsRetired` 的描述自此也印 rests-on（Codex 第 2 列：同一族的第三個生產者）。**R26（D73；R25 verify DA 第 17 列 MEDIUM、requirements 第 21 列）**：venue 的 `fieldsLostByMerging` 自此也比 references——R25 之前那一份**完全不比**、`mergeVerdicts` 只搬 verdict 欄位，被併 venue 的 `paginated` judgement 與 rests-on digest 在純量相同時隨檔案靜默消失，正是本列 R21 D60 為 rename 關掉的「程式編輯銷毀判定編輯的產物」在合併面的同型；三份遺失偵測的訊息自此說出「倖存者有正規化後相等、位元組不同的一筆」（第 14 列） |
| `update-venue --paginated`（#406） | AI | 「本刊是否使用頁碼」是判定，必附 judgement 與 rests-on |
| `update-venue --authorize`／`akashic_update_venue.authorize`（#554） | AI | 「這個名字是本刊的對外形」是判定——與 `authorize-names`（#81）同一句話換到 venue。語意是同 `WritingSystem` 原子替換（不是 append：「每書寫系統至多一個」對 470 筆機械值必擋 append）；被換下來的舊指定**移出 authorized、留在 names、不標 variant**——程式不替呼叫端多說「它是異寫」（R1 verify D1）。**不留 judgement**（與同列的 `authorize-names`、#471 `add_variant` 一致）——這是**有記錄的裁決不是遺漏**：三個名字分類面要不要留、留什麼形狀一次裁（#564）；在此之前這一格對「留 verdict」的義務是**未兌現的**，而它的代價住在上面 #553 那列（合併端分不出判定與機械值）。落地是程式（入口的輸入驗證整批拒絕零寫入；名字內容的不變式住在 `Venue.validate()`——五條的規範文字只在 `docs/store-format.md` §5.7（這裡不複述：R6 verify 第 10 列抓到本列是七份副本裡第一個分岔的）——所有寫入者存 canonical，D8；分割互斥同樣由 `Venue.validate()` 擋、不重造） |
| `record-divergence`（#77） | AI | 「當場記錄而非當場判斷」——記下判定尚未做出，本身也是判定型工作的一部分 |
| `rename`／`rename-person`（#35／#232／#395） | 程式 | 改名不改身分；參照集合由封閉列舉逐條窮舉、可機械檢查。**#554 R21／R22／R23（D60／D63／D62／D64）**：目的鍵上已有 verdict（三種 holder 與 quarantined 檔都掃——quarantined 檔以**位元組**比對 `<kind>:<newKey>`，不含空白、YAML 折行打不斷；R22 的行級比對被折行擊穿，R22 verify 四席同指）即具名拒絕、零寫入——程式編輯不得銷毀判定編輯的產物；唯一的收攏是完全相同（含 judgement 與 rests-on）的重複，零資訊損失（R21 verify 第 18 列：D60 曾只記在 `resolve-divergence` 列，本列才是它的家）——**「完全相同」是位元組層的話**（R24 D65，`ProvenanceReference.byteExactKey`；R23 verify Codex 第 1 列：R23 以合成 `Hashable` 當鍵，Swift `String` 的相等是 canonical 的，NFC／NFD 兩種拼法被折成一筆——R16 D42 修過同一個缺陷的另一處）；留下的同鍵異 judgement 由 `StoreHealth.duplicateVerdictRecords` 報（D64）——rename 那一步零損失，下一次合併會收攏、回報在合併的 `verdictsCollapsed` |
