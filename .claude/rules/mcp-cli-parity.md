...
| `akashic_resolve_organizations` | `resolve-organizations` | ✅（#304 移轉；CLI-only 表「候補缺席（重啟訊號已觸發）」格的補齊——同 `import-wos`／#290 的移列形） **#628 重新確認，裁決不變、契約有改**：作者位候選所在的 work 無法唯一定位時，MCP 的 apply／reject 整批拒絕、列表以 `unlocatableCitekey` 標出；CLI 的篩選式批次排除並另列。 |
