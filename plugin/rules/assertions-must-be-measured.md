...
| 就地重述讀不到的規範 | 對兩份 private 規則的重述失真，型態：漏證據項、把開放清單寫成封閉、漏掉識別碼例外、把「觀察」寫成「判準」、加上原文沒有的排序 |
...
| census 的 marker 解析現與讀端一致（19 格 fixture，1 格已知分歧） | `plugin/skills/akashic-literal-campaign/scripts/tests/store-marker-parity.sh`（需先 `swift build`；拿真的 CLI 當 oracle） | 2026-08-21 |
| 上一句那張矩陣真的會紅 | `plugin/skills/akashic-literal-campaign/scripts/tests/marker-parity-mutations.py` → 7/7 | 2026-08-21 |
| 本 plugin 經由**公開**的 marketplace 發布 | `gh repo view PsychQuant/psychquant-claude-plugins --json isPrivate` → `false`；`README.md` 的安裝指令即 `claude plugin install akashic-mcp@psychquant-claude-plugins` | 2026-08-21 |
