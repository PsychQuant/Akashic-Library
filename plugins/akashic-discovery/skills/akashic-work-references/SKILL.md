...
- 結束碼為 0，但 `refs.json` 的 `contract` 不是 3 → 見「開始前」第 1 點，停。
- 讀 `warnings`，每一則都要處理。warning 裡的位置寫成「第 L 行（PDF 第 P 頁）」：告訴使用者時給 **PDF 頁碼**，那是他對照得到的。行號是 `paper.txt` 的座標：不要給使用者，也**不要拿去讀 `paper.txt`**（見上方「不要直接讀」）。
  - **「有 N 個參考文獻標題候選都帶著條目」**：warning 列出其餘每一段的條目開頭數。其餘任一段有條目開頭，就**停下來**，把各段的 PDF 頁碼與數目告訴使用者，請他確認參考文獻清單從哪頁到哪頁——那可能是目錄或附錄，也可能是這份清單被切成了幾段。在他回覆之前不寫入。
...
同樣先看結束碼（非零 → 讀 stderr，多半是某個 `--openalex` 檔不是 OpenAlex 的回應，停），再看 `contract`（不是 3 → 見「開始前」第 1 點，停）。
...
寫入前，**用當下的 store 重跑一次 `nominate`**（先前那次到現在，store 可能被別的 session 改過）。輸出的 `contract` 要是 3，而且沒有「被隔離的檔案」warning。然後逐筆檢查**每一筆要寫 `cites` 的條目**，依它的類別用不同的條件（R5 D1：R4 把兩類套同一組條件，「已在庫只連」的 `inStore` 依定義不為空，永遠過不了）：
...
- library（D5）：**一律不自動掛**。用 `akashic_libraries(action: "list")` 讀出種子所屬每個 library 的名稱與描述，在乾跑裡列給使用者，由他逐一指定要掛哪個 library、掛哪幾筆（每次執行問一次）；種子屬於 0 個 → 不掛、不問。使用者指定之後，照 [`source-of-truth-over-consent`](../../rules/source-of-truth-over-consent.md) 核對：
  ...
  4. 描述空白、或看不出是上面哪一類 → 這一次**不掛**。把 library 的名稱與描述列進報告，請使用者先說明它屬於哪一類（#642 row 3）；同意本身不構成依據。使用者說明之後，下一次執行照對應的類別處理。
...
- **本機真實 PDF 的量測**（R5，169 份，只記數字）：切得出 124 份、5,519 筆。估計值（全文中以 `姓, 名縮寫.` 開頭、3 行內帶 `(年份).` 的行數）≥ 20 的 69 份裡，條目數低於估計值 80% 的有 2 份，都有會讓本 skill 停下來問的 warning：一份是「標題之前同頁有 20 個完整條目」（雙欄讀反到標題之前）；另一份是「清單停在圖表標題、之後還有 20 個」——那 20 筆經 R5 verify 查證是同一 PDF 裡**另一段**的清單，這則 warning 是誤停，而這份檔案真正的缺陷（一筆吸進了清單之後的正文，只有「略過」提示）沒有專門的 warning。**估計值有盲區**：124 份裡有 38 份的估計值是 0（不用 `(年份).` 的寫法），上面的篩選對它們沒有作用。
- **接受的啟發式模糊**（R5 verify 列出、判定不修，寫在這裡讓讀的人知道）：
  - 接續會越過補充資料、致謝等結束標題：清單真的在那裡結束、而之後恰好有排序接得上的完整條目時，會被當成接續；
  - 年份後接兩個字再接數字以外的表格列（`(2003) United States 120`）仍可能被當成條目；
