...
| # | finding | 重現結果 |
|---|---|---|
| ① | `oracle-precondition-control` 挑毒化目標時沒有 `has_swift` 過濾 | **今天為假**——ROBUST 三支守衛沒有任何一支需要 Swift |
| ② | `baseline：{_n} 支皆綠` 沒跟上 R67j | **成立**——訊息說 2 支，實際驗了 10 支 |
| ③ | `crossed` 是不歸零的閂 | **成立**——跨 2 個標題仍說「下一個標題之後」 |
| ④ | `#` 偵測不要求後接空白 | **成立**——行首 `#407 R33 的量測：` 被當成標題 |

**④ 是誤傷不是漏報，所以最該修**：本 repo 的規則檔滿是行首的 `#NNN`，而一張本來就
屬於當前標題的表會因此被判成 `borrowed`。改成 CommonMark 的 ATX 判定（`#{1,6}` 後接
空白或行尾）。

**③ 的尖點不在判定，在訊息**：那句話叫使用者「把宣稱搬到那個標題上」，而實作只有一個
布林、從未記下**是哪個標題**——訊息承諾了一個具體動作，實作拿不出它需要的資訊。改成
記下跨過的標題原文：跨 1 個時說「在「X」之後」，跨 ≥2 個時說「隔了 N 個標題（X … Y）」。

**② 落在一支專門抓「宣稱的數字沒跟上實際狀態」的 harness 自己身上**——R67j 把 baseline
驗證從固定 2 支擴成 `tested` 全部，而訊息沿用舊的 `_n`。

**① 修的不是缺陷，是不一致**：harness 的 `tested` 有 `has_swift` 過濾而目標挑選沒有，
同一個 commit 的兩半各說各話。今天選不到跑不動的守衛，但那個不一致會在**加入第一個
Swift 依賴的 ROBUST case 時**安靜變成缺陷——而唯一實際在跑的 CI 正是無 Swift 的 ubuntu。
順帶讓「過濾後沒有任何可毒化的守衛」出聲而不是靜默通過。

兩個新負控：跨兩個標題要具名兩個（須紅）、行首 `#407` 不得誤判（**須綠**，且已驗非空
——回退 ATX 判定它就紅）。

負控 52/52（43 須紅 ＋ 7 須綠 ＋ 2 後設檢查）；從 pre-push 導出 20 支、失敗 0。
...
| 子點 | 實測 |
|---|---|
| fence 內的示範表被當成真的表 | 放一張 1 列的示範表在 ``` 裡，真正的 3 列表在後面 → 回報 **1** |
| 緊鄰的第二張表被併入計數 | 2 列的表後面緊接一張表 → 回報 **5** |

修法：掃描時跳過 fence 內容；計數遇到第二張表的分隔線就停（並把它的表頭那列扣回去）。

**接著是這一輪最值得記的部分。** 為這兩個修正寫的兩個 ROBUST（須綠）case，跑起來
都是 ✓——然後**回退修正，它們仍然是 ✓**。它們是空的：注入打在檔案較早的另一張表上，
而受檢的宣稱綁的是後面那張裁決表。**位置錯了，控制組就什麼都沒測到，而它看起來
完全正常。**

改對位置後，回退 fence 修正 → fence 那格紅；回退相鄰表修正 → 相鄰表那格紅。逐一
回退（不是一起退）才知道哪一格對應哪一個修正。

**第三件是自審抓到的**：harness 只對兩支守衛驗 baseline。其餘守衛若在注入**之前**
就已經紅，它們的控制組會平白通過——控制組宣稱「注入造成了紅」，而紅早就在那裡。
實測：先把一個裸數字注進規則檔，**10 個 numbers 控制組全部照樣報 ✓**。

改成逐守衛驗 baseline（跑一次未注入的乾淨 copy），髒的就在跑任何 case 之前攔下並具名。
**成本誠實記下：這支守衛從 16.7 秒變成 42.0 秒**（＋25 秒）。換到的是 42 個控制組
不再可能對著髒 baseline 空轉。

兩個自檢（oracle 前提、逐守衛 baseline）的負控都在 `oracle-precondition-control.py`。

負控 50/50（42 須紅 ＋ 6 須綠 ＋ 2 後設檢查）；從 pre-push 導出 20 支、失敗 0。


## R67i — 手寫的對照表每次都會漏，而漏掉的形式是沉默

跨模型審查（correctness 席）在 R67d 的列數檢查上找到兩條 MEDIUM，兩條都自己重現過：

**① `_CN` 中文數字表不完整。** 實測 `十五`／`二十`／`二十一`／`三十` 全部回 `None`
→ 靜默略過。手寫表的問題不是「這次漏了哪幾個」，是**它每次都會漏，而且漏掉時什麼都
不會發生**。改成位值解析（`十`／`百`，涵蓋 1–999），實測 `一百零五` → 105。

**② `_rows_after` 會跨過標題借用下一節的表。** 實測：一個宣稱了「現有 3 列」卻自己
沒有表的標題，會抓到下一節那張 2 列的表來比對——兩個不相干的數字被湊成一對，而結果
看起來完全正常。

修完後又多做兩件，都是被守衛自己逼出來的：

**③「解析不出要報，不得略過」原本是死碼。** 加了那條分支之後才發現：`_COUNT` 的字元類
只收 `_num` 認得的字，所以「匹配得到卻解析不出」**永遠不可達**——一段沒有輸入到得了的
程式碼，讀起來卻像一道防線。把字元類擴到 `廿`／`卅`／`萬`（真實會出現、`_num` 不處理），
那條路徑才有東西走得到，負控也才寫得出來。

**④ R67c 的重複掃描第一次遇到真的重複就抓到了。** 新的「不得借用」與舊的「完全沒有表」
輸出逐字相同。解法不是把它們列為刻意的一對，而是**讓守衛真的區分這兩件事**——它們的
診斷不同：前者要把宣稱搬到對的標題上，後者要補回本節的表。守衛能說出差別之後，兩個
case 自然可區分。

負控 48/48（42 須紅 ＋ 4 須綠 ＋ 2 後設檢查）；從 pre-push 導出 20 支、失敗 0。
...
實際已是 19 支。重量：**19 支 99.6 秒**，最大一支 `marker-parity-mutations.py` 69.4 秒
（70%）。

端到端 pre-push 量到 **778 秒**（rc=0），但那次有一個 21-agent 的審查 workflow 在同一台
機器上跑。**所以寫進表裡的不是 778，是「778（見下方條件）」＋一段說明**：支數 15→19 只
多了 22.5 秒的守衛時間，其餘要歸給 CPU 競爭與 `swift test`；一個沒有條件的 778 會被下一個
讀者拿去跟 337 相減，得出一個沒有人量過的結論。乾淨環境的重量待補。

**R67g（掃描範圍）**：`measured-numbers-audit.py` 只掃兩個 rules 目錄，而 `CLAUDE.md`
**是每個 session 都被自動注入的檔案**——過期數字在那裡的影響面比任何規則檔都大，它會
出現在往後每一輪的 context 裡而讀者沒有理由懷疑它。實測缺口 2 處、1 處裸（正是 R67f
剛加的那一列），小到補起來幾乎免費。

擴範圍時**引入了一個真的回歸，並被既有負控當場抓到**：原本「一個檔都沒找到就報錯」的
檢查看的是**聯集**，加入 CLAUDE.md 之後，兩個 rules 目錄整個消失時聯集仍非空 → 路徑
打錯或規則被搬走不再有任何跡象。改成**逐來源**檢查，並具名是哪一個歸零。

> 這與 `trigger-coverage` 當初從「聯集」改成「逐對」是同一個修正。**同一個錯誤形狀在
> 這個 repo 出現第三次**（第一次是 census-parity 的檔案集合聯集、第二次是 trigger
> 覆蓋的聯集），所以它不是個案：只要一個檢查的輸入來自多個來源，「有沒有東西」就必須
> 逐來源問，問聯集永遠會在其中一個歸零時說謊。

負控 45/45（39 須紅 ＋ 4 須綠 ＋ 2 後設檢查）；從 pre-push 導出 19 支、失敗 0。


## R67e — oracle 自己的前提要被檢查（因為它的失敗會外溢）

R67 讓 ROBUST 負控拿「守衛在未注入 copy 上的輸出」當 oracle、要求逐字相同。那個機制
有一個沒寫下來的前提：**守衛輸出是決定性的**。若哪天某支開始印時間、耗時、temp 路徑
或走訪順序，逐字比對就會偶發變紅。

**偶發的紅比漏報更貴**，而這是本輪裁決的全部理由：漏報只是少抓一件事；假紅會讓維護者
學會「這支有時候會紅，重跑就好」——而那個習慣不會停在這一格，它會套用到**所有**守衛
身上。這是本 harness 唯一一個「失敗會傷到其他守衛」的機制，所以它自己要被檢查。

實作是每支 ROBUST 守衛多跑一次未注入、比對兩次（實測 2 支、約 2 秒）。前提不成立時
**不假裝那一格通過**：計數會從 43 掉到 42，而不是靜默略過。反向證實跑過（讓一支印
`os.getpid()`，各 `with_copy` 是獨立 subprocess 所以必不同）：報前提不成立、那格不計入、
`rc=1`。

依 `zero-instance-guards.md` 加第 6 列（private repo，外部讀者取不到）——前五列的失敗
都關在自己那一格裡，這一列會外溢，理由與它們都不同。

**而加完那一列，R67d 的守衛在下一個變更就抓到了它要防的東西**：標題仍寫「現有 5 列」
而表已有 6 列。這一格是刻意留著沒改、跑一次守衛看它報，然後才修——一個守衛在它落地
的隔一個 commit 就派上用場，值得記下來。

負控 43/43（37 須紅 ＋ 4 須綠 ＋ 2 後設檢查）；從 pre-push 導出 19 支、失敗 0。


## R67c／R67d — 把 R67b 的形狀一般化，然後它在別的檔案上抓到一個真實例

**R67c（免費的一般化）**：R67b 那對 case 之所以不可區分，機械徵候是「輸出逐字相同」。
既有迴圈本來就跑完全部 35 個 case，所以分組只是多一個 dict——把它常設化，未具名的
重複即報。實測 0 組。

這是零實例守衛，依 `zero-instance-guards.md` 加了第 5 列，理由與前四列都不同：
前四列的代價都落在**被守衛的東西**上，這一列落在**守衛自己身上**——一個重複的 case
會讓計數 35 → 36、多印一個 `✓`，讀成「又多檢查了一件事」。反向證實時看得很清楚：
複製品自己印 `✓ 具名`，靠掃描才抓到。

**R67d（同一個形狀，但有實例）**：加完那一列，`zero-instance-guards.md` 的標題仍寫著
「封閉列舉——**現有 3 列**」——而表當時已有 4 列。它在表長到第 4 列時就過期了，
沒有任何跡象。所以這不是零實例，是**當場抓到的實例**，不必進裁決表。

守衛併進 `measured-numbers-audit.py`（同一個主題：散文裡的數字 vs 現實），**前件是量
出來的**（該規則第 2 列的教訓）：

| 前件 | 受檢 | 不符 | 誤傷 |
|---|---|---|---|
| 寬版（任何行的「只有 N 條」） | 9 | 2 | **1**（散文的「只有一條」被 10 行外的表誤配） |
| 收窄成**只認標題行** | 3 | 1 | 0 |

標題是宣告表的邊界的地方，散文不是。三個負控：計數漂移、錨不存在（標題宣稱了列數
卻找不到表——刻意與「表有 0 列」分開，不折成同一個值）、以及**須綠**的那個：散文裡
的「只有 2 條」不得被誤判。

**順帶的自證，發生了兩次**：R67c 的新註解提到 `.claude/rules/...`（private repo，
外部讀者取不到）而沒在同一行揭露取用限制，被 `rule-prose-guards.py` 的第 2 項當場
擋下。**然後這一段——記述那次違規的這段本文——又犯了同一條**，因為它引用了那個路徑
而同樣沒揭露。兩次都是在寫「守衛要能抓到自己人」的同一輪裡被自己的守衛抓到。

負控 43/43（37 須紅 ＋ 4 須綠 ＋ 2 後設檢查）；從 pre-push 導出 19 支、失敗 0。


## R67b — R67 自己沒修乾淨：兩個 parity case 是同一個測試做了兩次

R67 給三個 parity case 各自補上命令名，宣稱它們因此可區分。**送審前自己量了一次，
發現其中兩個都針對 `fmt`**——加同一個 `` `fmt` `` 對它們毫無區分力。更進一步比對，
它們的輸出**逐字相同**。

讀原始碼才看懂為什麼：其中一個的注入是另一個的注入**再加**「把 `fmt` 種進理由欄」，
而 R60 的主張正是**那個加法不得改變任何事**。所以輸出相同不是缺陷，是它要證明的
結論——而任何字串斷言都分不開它們，因為沒有字串可分。

可區分的斷言是不變式：`out(理由欄版) == out(純散文版)`。守衛若退化成認整列
（理由欄也算），理由欄那個 case 會少掉「完全沒提到」而純散文的保留 → 兩者分岔 →
不變式紅。這個反向證實實地跑過（把守衛的 `split('|')[:2]` 放寬成整列）。

**這一格的教訓與 R67 主文同型、方向相反**：R67 說「通用斷言測不出哪個注入」，
R67b 說「有些 case 的可區分性根本不在輸出裡」——它在**輸入之間的關係**上。逐 case
補字串是對的一半；另一半要問「這兩個 case 的差別會表現在哪裡」，而答案可能是
「哪裡都不表現，那正是它要證明的」。

發現方式值得記：不是審查抓到的，是**我自己在送審前跑了一支交叉檢查**（每個新斷言
是否只在自己的注入下出現）。那支檢查花不到一分鐘，而它抓到的是我上一輪剛寫的東西。

負控 39/39（35 須紅 ＋ 3 須綠 ＋ 1 不變式）；從 pre-push 導出 19 支、失敗 0。


## R67 — 負控的斷言要能區分「哪個注入」與「通過得對不對」

跨模型審查指名兩個 `multi` 負控**斷言完全相同**（都只有裸的 `★`）。`★` 是任何一列
不符都會印的通用標記，所以：把兩個 case 對調不會被發現，而一個與注入無關的破壞
（改某列的 expected、`RANGES` 差一格）同樣會印 `★` 而被記成「抓到了」。

順著掃了整份 harness，另有 **六組斷言被 2–3 個 case 共用**。它們不像 `★` 那麼糟
（訊息是具體的），但共用同樣讓「哪個注入配哪個 case」測不出來。守衛的訊息本來就
帶著具體項目名，所以修法只是把它加進斷言：

| 斷言 | 改成 |
|---|---|
| `★` ×2（multi） | `分歧數：11`／`分歧數：4`（實測 inTable 反轉 11 列、改看最後一個 scalar 4 列） |
| `規則檔裡完全沒提到` ×3（parity） | 各自加 `` `fmt` ``／`` `fmt` ``／`` `doctor` `` |
| `新欄位未經裁決` ×3（ratchet） | 各自加 `ghostEdge`／`seeAlso`／`brandNewEdge` |

**剩下那一組是對的，但它的斷言本來就測不到自己要測的東西**：兩個 ROBUST（須綠）
case 都斷言守衛的通過訊息 `三面皆同步`——而通過訊息對所有 ROBUST case 都一樣。更糟
的是它**證明不了「通過得對」**：切片若靜默截斷而剛好沒少掉命令名，守衛照樣印通過。

改成拿**同一支守衛在未注入 copy 上的輸出**當 oracle：純重排不得改變任何一個字，
含 `MCP 30｜CLI 43｜橫切 2` 這行計數。**現算而非寫死**——把 43 寫進 harness 會製造
第二份會與 `CLI.swift` 分岔的規格，正是本 issue 在修的形狀。

新機制自己有反向證實：注入一個「不改結論、只多印一行」的編輯，`rc` 維持 0（舊斷言
會放行）而 drift 被抓到。

從 pre-push 導出 19 支、失敗 0；audit 負控 38/38（35 須紅 ＋ 3 須綠）。


## R66 — 跳脫表補齊（`\\_`／`\\N`／`\\L`／`\\P`／`\\a`／`\\v`／`\\e`）

R63 建 `_ESC` 時只放了常見的幾個，漏掉的會落到 fallback `_ESC.get(e, e)`——**輸出字面
的那個字母**。YAML 雙引號純量的完整表還有 NBSP（`\\_`）、NEL（`\\N`）、行／段分隔
（`\\L`／`\\P`）、bell／vtab／esc。

補齊後 parity case 12 → **16**，全部一致。真實 store 的數字不變。

**這是第四輪修同一個函式**（R62 建、R63 跳脫語義、R64 `\\U`、R66 補表），每一輪都是
跨模型審查指名。共同形狀：**我每次只補了「我想得到的那幾個」**，而 YAML 的跳脫是一張
有明確定義的封閉表——查表比想像可靠。

從 pre-push 導出 **19 支、失敗 0**；audit 負控 38/38。
