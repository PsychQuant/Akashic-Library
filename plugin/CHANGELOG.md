...
- **第七次：把不可查核的斷言換成可查核的，才是斷開連鎖的方式**（#407 R25e，c18 的 DA 席指名）：R25c 那一列的**結尾金句**是「六次全部由跨模型審查抓出——**作者自審每一輪都認為那段話已經夠精確**」。後半句對**心理狀態**下斷言，無從查核；我沒有自審記錄，那是推論。**而它偏偏寫在描述前六次錯誤的那一列裡**。
  改成 git 查得到的事實：六次全部由跨模型審查抓出，且**都在該段話 commit 之後**——每一次它都通過了作者當下的檢查才進版控（六個 `#407 R…` 更正 commit 各有 hash 與時間戳，各對應一段先前已 commit 的文字）。
  同輪修 DA 的第二條（LOW）：「R24f 的原文在本檔上方」沒給行號，而同段落緊鄰的兩處都給了可重跑的查核。補上 `grep -n` 與行號（**581** vs 本行 598）。
  **斷開連鎖的不是「再寫一句更小心的話」，是把斷言換成可查核的形式**——這一輪的兩處修正都做到了，而且新句本身也被查過（六個 commit 的 hash／時間戳、兩個行號）。
