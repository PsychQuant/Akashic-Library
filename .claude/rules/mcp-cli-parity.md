...
| `akashic_resolve_organizations` | `resolve-organizations` | ✅（#304 移轉；CLI-only 表「候補缺席（重啟訊號已觸發）」格的補齊——同 `import-wos`／#290 的移列形） **#628 重新確認，裁決不變、契約有改**：作者位候選所在的 work 無法唯一定位時，MCP 的 apply／reject 整批拒絕、列表以 `unlocatableCitekey` 標出；CLI 的篩選式批次排除並另列。**change `resolution-verdict-states`（#619）：org 族的未決寫入面兩面都缺席**——store 層接受 org holder 上的 `resolution-undecided`（三種 holder 同一道驗證與 format 19 閘），但 resolve-organizations 沒有 undecided 腿；缺口記 #643（`replace-endnote-and-zotero` 第 4 條：能力缺口記 issue，不靠「設計」帶過） |
