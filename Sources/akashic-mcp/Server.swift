...
"split_author": strArray("把一個作者位**拆成多個**（#443）：citekey:authorIndex:分隔符=理由。一個 literal 裝了兩個人時用它。**收分隔符不收拆好的名字**——後者等於讓呼叫端編造；收分隔符則拆出的每一段必然是原文的子字串。切出空段即拒絕；只作用於未歸戶的位置；拆出來的仍是 .literal（拆是形狀修正不是身分判定）。單獨呼叫，不與其餘腿組合——它改的是作者位的數量"),
        ...
        Tool(name: "akashic_update_venue",
             ...
             inputSchema: obj([
                ...
                "authorize": strArray("指定為對外形的名字。**不是 append**：authorized 每書寫系統至多一個（`AuthorizedNames.validate` 的內容約束），同書寫系統原本的指定會降成 variant、回報在 `demotedToVariant`；跨書寫系統（例如加一個中文刊名）才是 append。不在 names 的一併加進 names。這是 #553 攣生合併「authorized → variant」降級的精確逆操作——在此之前 authorized **沒有判定型寫入面**，唯一寫入者是 VenueBootstrap 取第一個名字的慣例（實測 470 筆），而那些機械值換不掉（append 會被「至多一個」擋）。#471 修了 variant 那一半、本參數修 authorized（#554）"),
