...
- [ ] 3.1 [P] 交付 spec requirement「An identifier SHALL live on the entity it identifies」。`Venue` 取得 `issn: [ISSN]`、`Organization` 取得 `ror: ROR?`，且 `Venue` 型別 doc comment 內逐一列舉欄位的那段同步更新。驗證目標——新測試以反射斷言兩型別的欄位集合含新欄位，且 `Venue` 的 doc 列舉與實際欄位一致。
- [ ] 3.2 [P] `Entry` 取得 `doi: [DOI]`／`pmid: [PMID]`／`isbn: [ISBN]` 結構化欄位，與既有 `Entry.fields` 並存（本任務不移除 `fields` 內的舊值，移除由遷移負責）。驗證目標——新測試斷言同時持有結構化值與 `fields` 殘留時，結構化值為正典。
