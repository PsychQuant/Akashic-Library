...
| `rename`／`rename-person`（#35／#232／#395） | 程式 | 改名不改身分；參照集合由封閉列舉逐條窮舉、可機械檢查。**#554 R21／R22（D60／D61／D62）**：目的鍵上已有 verdict（三種 holder 與 quarantined 檔都掃）即具名拒絕、零寫入——程式編輯不得銷毀判定編輯的產物；唯一的收攏是完全相同（含 judgement 與 rests-on）的重複，零資訊損失（R21 verify 第 18 列：D60 曾只記在 `resolve-divergence` 列，本列才是它的家） |
