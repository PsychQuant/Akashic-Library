...
### claim 與 interpretation 的哲學一致性由誰守（#267）

`tractatus-doc validate` 驗的**全部是結構**：locator 解析得開、每命題至少一條 relation、
`aspirational` 要有 issue history、rationale 跨命題唯一、`not_applicable` 的約束。
**沒有一項讀 interpretation 的內容。** 所以「這條 relation 的 claim 與該命題的
interpretation 是否矛盾」（#235 verify R2DA-1 抓到的 3.322 記號任意性一類）在機械層
完全不設防。

本節是那個缺口的承載機制裁決。**三個候選都只降低機率、都不保證**——先把這句寫在前面，
免得本節本身變成「已經有機制了所以站得住」的錯誤安全感來源。

#### 裁決：具名的第二讀者（候選 b 的強化版）

**corpus 的 relation 變更，其 verify ensemble 必須包含一個具名的哲學一致性席位**，
其職責是：拿該 relation 的 claim 去對照**同卷該命題的 interpretation**，回答
「claim 有沒有說出 interpretation 不支持、或與之相反的話」。

這不是新機制——**它是唯一實際抓到過東西的機制**（#235 R2DA-1、#255 的樣板級形狀，
兩次都出自 verify ensemble）。本節做的事只有一件：把「碰巧有人讀到」變成
「有人被指派去讀」。ensemble 的 lens 組成每輪會變，不寫下來就不保證下一輪還有人讀哲學。

#### 為什麼不是「作者自審 checklist」（候選 a）

不是因為 checklist 沒用，是因為**它要求的正是已被量測證明做不到的那件事**。

全域規則 `common-spec-prose-enumeration` 記著一個對照事實：`perspective-writer#7` 連續
四輪跨模型盲驗，**所有 blocking finding 全部出自跨模型驗證，而作者自審每輪都報「完全
符合」**。那條規則的結論是：

> 自審能驗「我打算做的有沒有做到」，驗不了「做出來的東西自己有沒有矛盾」——矛盾正是
> 作者看不見的那部分。**規格散文的自我矛盾要靠另一個讀者，不能靠作者自己再讀一遍。**

claim 與 interpretation 的矛盾**就是**散文的自我矛盾。所以 (a) 若作為承重機制，等於把
唯一有效的環節換成已知無效的那個。checklist 本身保留價值——它是上面那個席位的**任務
說明書**，不是它的替代品。

#### 為什麼不是機械化（候選 c），以及什麼條件下重啟

實測數字（#267 診斷）：`not_applicable` 共 **383** 條，**被 >1 條共用的 rationale = 0 種**
——383 段各自獨立的文本，**沒有可利用的重複結構**。關鍵詞比對在這種輸入上的假陽性率
沒有理由樂觀。

而 `.claude/rules/zero-instance-guards.md` 第 2 列已經裁決過同型情形：判準不是「要不要
寫」而是「前件多寬」，且**「沒有那次量測就不該動手」**。本 repo 另有一條記過的教訓：
被忽略的檢查比沒有檢查更糟——它讓人以為已經檢查過。

所以 (c) **現在不做**，但不是永久否決。重啟條件是可執行的：**對 20 條已知正確的
relation 跑一次比對，數它報幾條假警報**。有了那個數字才談要不要做；沒有那個數字就
動手，等於把 issue 自己寫的「可能過度工程」變成「已經過度工程」。

#### 這個機制守不到的地方（必須說出來）

- **它只在有 relation 變更時作用。** 已在 corpus 裡的 383 條不會因為本節而被重讀一遍。
- **它是人的判斷，不是閘。** 席位存在不代表那一輪真的讀出東西；ensemble 給的是機率。
- **它與 [`not_applicable` 的 claim 寬度](#not_applicable-的-claim-寬度255) 是兩件事。**
  那一節管 claim 是否**寬於** rationale 的根據；本節管 claim 是否與 interpretation
  **矛盾**。兩者都不可機械驗證、都由同一個席位承接，但判準不同，不得互相代替。
