...
| `akashic_files` | `file` | ✅ |
| `akashic_libraries` | `library` | ✅（#455 起 add／remove 兩面同走 `setMembership`：CLI 收多 citekey 是批次形、MCP 維持單 citekey——批次面的裁決見 CLI-only 表） **#628 重新確認，裁決不變、契約有改**：add／remove 指名的 citekey 重複或與另一筆 work 共用 id 時兩面都整批拒絕。 |
| `akashic_import_zotero` | `import-zotero` | ✅ |
| `akashic_import_wos` | `import-wos` | ✅（#290；#259 CLI-only 盤點唯一「需要」格的補齊——#206 鏡像判準）|
| `akashic_resolve_people` | `resolve-people` | ✅（#272 起兩面契約有記錄的差異：MCP 允許 apply+reject 組合（兩段式、按腿回報）；CLI 維持分兩次呼叫——互動面天然序列，組合是 LLM 批次 triage 的需求。#303 起兩面同步帶 `tier`（封閉四值 exact／confirmed-elsewhere／reorder／initials，信心降冪）：MCP 每列 `tier` 欄、CLI 按 tier 分組標頭與 `--tier` 篩選（裸 `--apply` 對寬鬆 tier 拒絕）、App 候選列標示；apply id 升三段形 `citekey:authorIndex:personKey`（釘 person，兩段 legacy 收）；否決抑制改與提名同一套正規化、淘汰而得的唯一命中在 reason 揭露——R1 verify 後 apply 語意有這些**有記錄的變更**，非純 additive。R3 裁決的**面不對稱**：tier 閘只在 CLI 篩選式批次（MCP per-id 顯式＋tier 可見，刻意不閘；tier-acknowledgment 參數列 follow-up）；rejected/applied 回音均三段 pinned 形。**#624 起另一個有記錄的差異**：淘汰而得的唯一候選（此位置其他人選已被否決，`eliminatedPairings > 0`）——CLI 的篩選式 `--apply` 排除它、逐筆列出並指路 `--judge`，全被排除時零寫入、非零結束；MCP 的逐 id apply **照寫**（顯式指名，理由同 tier 閘的面不對稱）。MCP 列表帶結構化的 `eliminatedPairings`；CLI 沒有 `--json`，同一件事以文字揭露——列表每列標「淘汰而得」、reason 帶「已被否決」、`--apply` 時另列排除清單。兩段 legacy id 若指到淘汰所得會被拒（只指到位置、沒點名人），適用於 MCP 的 apply／reject 與 CLI 的 `--reject`（CLI 的 `--apply` 只送三段 id），要用三段形；**#627 起 citekey 重複時一律不猜**：兩面的 apply／reject（兩段與三段 id）與 split／un-split／drop／attribute-org 整批拒絕，judge／refute 該筆具名略過，CLI 篩選式 `--apply` 排除並另列；「無法唯一定位」自 R2 起含與另一筆共用 id 的 work（`unlocatableCitekeys` 一個定義，各腿共用——寫入以 id 定檔，改它等於改兄弟的檔）；`PersonResolver.apply` 本身也不套用這些位置、以陣列位置就地改寫（最後一道防線）；App 裁決台的 accept 具名拒絕（App 列表沒有標記，第三面的已知差異）；MCP 列表以 `unlocatableCitekey` 先標出，CLI 列表以 ⟨citekey 重複或共用 id⟩ 標出——這兩面同一件事；confirm 的 verdict 只寫給真的改到的作者位，apply 以 `notApplied` 回報；judge 同一次呼叫把同一作者位判給兩個人整批拒絕。**有記錄的差異**：judge／refute 全部略過時 CLI 非零結束、MCP 照常回成功由呼叫端讀 `skipped`（同 #624 全數排除的同形差異）；作者位早已歸給同一個人時，兩面同契約：已有同一句理由的逐篇判定＝no-op 成功（`alreadyJudged`），理由不同則具名略過；由其他規則歸戶的具名略過（升級面缺口 #636——**自 change `resolution-verdict-states` 起不成立**，見本列 (c)）。**#635 起 judge／refute 各自單獨呼叫**：兩面都顯式拒絕它們與彼此或 apply／reject 的組合（先前靠提早返回隱含達成，其餘腿被靜默丟掉、仍回成功）——同結構腿（#443）的既有契約。造出這些狀態的儲存層路徑見 #631。同型但不屬 resolve-people 族的寫入面見 #628。tier 閘看排除後的套用集。刻意不給 CLI 關掉排除的旗標——那會把排除變回一鍵可關的預設；查過未決的配對兩面都看不到，繫於 #619）。**change `resolution-verdict-states`（#619／#636，2026-09-25）重新確認，裁決不變、契約有改**：(a) 兩面同時新增 **undecided 腿**（CLI `--undecided`／`--rests-on`，MCP `undecided`／`rests_on`，同走 `recordUndecidedAuthorships`），記下「查過、判不出來」與查了什麼——兩面同契約：整批拒絕六類、逐筆略過四類、完全相同＝`alreadyRecorded`、單獨呼叫；上一句「兩面都看不到」自此不成立。(b) 查過未決的配對在列表揭露（MCP `undecidedChecks`／`counts.undecided`／`undecidedTotal`，CLI「查過未決 N 次」與四態計數行）；**CLI 的篩選式 `--apply` 排除它、另列並指路 `--judge`，MCP 逐 id apply 照寫**——與 #624 淘汰所得同形的有記錄差異，理由相同；App 裁決台（第三面）同樣標「查過未決 N 次」，Accept 是逐筆顯式、照常可用（R1 verify：初稿漏了這一面）。(c) `judge`／`refute` 對「已由提名層同向判過」的配對改為寫入並存的逐篇判定（回應帶 `coexistsWith`，需 store format ≥ 19），兩面同契約。(d) R2／R3 verify：judge 與 attribute-org 對 literal 作者位照常歸戶、理由存不進去時該列帶 `verdictNotRecorded`（CLI 印 ⚠），兩面同契約；同批 refute 碰撞具名略過；undecided 腿的整批拒絕另含三個上限（200 個 id、20 個 digest、4,096 位元組）。**#597 起多一個有記錄的差異**：CLI 列表模式（不帶 `--apply`）的 `--citekey`／`--person`／`--tier` 收窄候選列與歧義條目，並印出收窄後／全部的計數；MCP 的列表面沒有對應參數。理由同 `--rows` 那格：CLI 的輸出進人的終端機，逐篇查證要的是「這一篇還有誰被提名」；MCP 呼叫端拿得到 `candidateTotal` 與逐列 id，可以自己篩，而為它加參數會讓一個 LLM 呼叫端以為沒列出的候選不存在——那正是 CLI 用「全部 M 個」那一行防的誤會，MCP 面沒有對應的一行。MCP 呼叫端若需要逐篇收窄，要重開這一格並一起裁位元組預算）|
...
| `akashic_tag` | `tag` | ✅（#219；契約已收斂——零參數兩面皆拒，守衛下沉 service 單一路徑，#258）。**#628 起 tag／link／set-status 在 citekey 重複或與另一筆 work 共用 id 時兩面都拒絕**（同走 `requireEntry`）|
...
| `akashic_resolve_organizations` | `resolve-organizations` | ✅（#304 移轉；CLI-only 表「候補缺席（重啟訊號已觸發）」格的補齊——同 `import-wos`／#290 的移列形） **#628 重新確認，裁決不變、契約有改**：作者位候選所在的 work 無法唯一定位時，MCP 的 apply／reject 整批拒絕、列表以 `unlocatableCitekey` 標出；CLI 的篩選式批次排除並另列。~~**change `resolution-verdict-states`（#619）：org 族的未決寫入面兩面都缺席**~~ → **change `org-undecided-leg`（#643）起兩面同時新增 undecided 腿**（CLI `--undecided`／`--rests-on`，MCP `undecided`／`rests_on`，同走 `recordUndecidedOrganizations`；單獨呼叫）。id 是 `<列表的 id>@<orgKey>=說明`：在每個 `@<StoreKey>=` 的位置試切，前綴必須恰為這次列表的已知 id，恰一個才收（使用者 2026-09-25 裁決——org 的 id 帶 literal，第一個或最後一個 `=` 都會切錯）；歧義條目也帶 id、可逐個 org 記。兩面同契約（整批拒絕、逐筆略過、`alreadyRecorded`、呼叫層三個上限加單筆 id 長度上限、person／organization 同 key 兩列都在列表上時整批拒絕、format ≥ 19——R2 verify 起 CLI help 與 MCP 描述逐條對齊）。揭露：MCP 候選列與歧義條目帶 `id` 與 `undecidedChecks`（歧義條目是 orgKey → 次數）、頂層 `undecidedTotal`（候選配對，與 CLI 四態計數行同一個定義）與 `ambiguityUndecidedTotal`（CLI 另印一行，同一個定義）；CLI 列表逐列印 id 並標「查過未決 N 次」，消毒改了 id 時標出它不能逐字送回。**有記錄的差異**：CLI 篩選式 `--apply` 排除查過未決的候選並另列、全數排除時零寫入非零結束；MCP 逐 id apply 照寫；`--reject` 兩面都不排除（否決是一個判定）。**與 #624 不完全同形**（#643 R1 verify）：people 的 CLI 有 `--judge` 這條逐 id 的路，org 的 CLI 沒有逐 id 的 apply——查過未決的 org 候選在 CLI 上歸戶不了，只能走 MCP；缺口記 #647。`--undecided` 不接受 `--holder`／`--org`（id 已點名，收窄沒有作用） |
...
| `resolve-divergence` | 有理由缺席 | in-code 既有裁決（`AkashicService.recordDivergence` doc）：消歧含合併＋全庫改寫＋刪檔，tracked+clean 前提與人工確認屬 CLI／App 互動面。**#553（2026-09-11）重新確認，裁決不變**：該輪讓本命令多收一個 shape（venue），而缺席的理由是**操作的性質**（不可逆、要乾跑逐筆過目、要 git 工作樹乾淨），與它收幾個 shape 無關——多一個 shape 不會讓那三件事對 LLM 消費面變得可承擔。`record-divergence` 那一面在 MCP 表已有對應（`akashic_record_divergence`），本輪的 `byShape` 擴充**兩面同時生效**，沒有新的不對稱。**#558 R2（D86）重新確認，裁決不變、契約有改**：tracked+clean 前提自此涵蓋**會被改寫的檔**（倖存者、被改指的 entry、塌縮／改名的歧異記錄），不只被刪的——「要乾跑逐筆過目、要 git 工作樹乾淨」那三件事因此對 LLM 消費面更不可承擔，缺席的理由變強不變弱。**#558 R4（D87／D88）再確認，裁決不變、契約有改**：閘多兩種拒絕——HEAD 解析不到（unborn、orphan 分支、HEAD 指向不存在的 ref，各自的訊息）與 index 標了 `assume-unchanged`／`skip-worktree`（R1–R3 三代都對它 fail-open，R3 verify security 席真 binary 刪檔重現）；拒絕清單的上限改成自己的常數，超限時補救句叫人一次 commit 全部 |
...
| `validate` | 有理由缺席（理由於 #416 換過）| **原理由「讀取檢查由 `akashic_doctor` 覆蓋（功能重疊）」被量測否掉**：`Entry.validate()` 在全樹只有一個呼叫點（CLI），`doctor()` 與 App 面各 0。**落差是 warning 一族**（#416 R1 更正：原本寫「有一條是 error 級」，實測那條到不了——load 的 quarantine 先擋下，而 quarantine 本來就在 `StoreHealth` 裡）。#416 把 per-record 驗證抽進 `StoreHealth.perRecordIssues`，兩個消費面因此都拿得到——**現在**才真的重疊。CLI 面維持獨立命令的理由改為：它**不加面級截斷**、逐則列出，而 MCP 面截斷 20 則（輸出進 LLM context，呼叫端無法在收到後丟棄已付的代價，#236 的既有威脅模型）。這是**呈現粒度**的差異，不是能力缺席。**#554 R24（D66）重新確認，裁決不變、理由收窄**（R23 verify Codex 第 2 列）：R14–R26 陸續在 `validate()`／`StoreHealth` **產生訊息時**就套了 per-record 上限（`Entry.perRecordWarningCap`），**只在組合式的六族**：venue 名字內容、venue 的 names／authorized／variant 近重複、**person 近重複**（R26 D72、#576）、重複 venue 邊、confirmed literal、重複判定記錄——各族每筆記錄至多 20 則加一句概括；venue 名字近重複與 person 近重複另各有**整筆 100,000 對**的求值總量上限（R18 D52／R27 D76：venue error 級整筆拒絕、person warning 級只計數並自 R28 起小組先評估，R27 verify 第 25 列指出這一格只列了四層）。那是**求值的安全上限**（讀取路徑對未信任的 store 內容跑，R14 verify security 第 18 列）：這六族的則數是每筆記錄的**組合**（配對數／組數），會被一筆記錄撐爆；死 verdict、矛盾 verdict、本機缺存檔、拆分後孤兒 verdict、拆分／移除記錄這幾族**沒有上限**，它們的則數是每筆 reference／配對／記錄各一則——與那筆記錄持有的資料項數**線性**（1,352 筆 verdict 的 holder 全退役就是 1,352 則，不是「每筆至多一則」），上限的理由（組合數遠超資料項數）對它們不成立、上界就是資料項數（R25 D70 → R26 D72；R24 verify 第 12／14／37 列：R24 在 App help 寫成「每筆記錄每族」、對五族以外為假；R25 verify DA 第 2 列 HIGH、第 6／11／15 列：R25 寫「五族」漏掉 person 近重複——它當時是組合式且無上限、真 binary 一筆記錄 19,900 則——又寫「其餘線性、每筆至多一則、撐不爆」，對六族全假。兩輪各換掉一句假話又寫了另一句：這一格的措辭自此逐族點名、不再用「其餘」）。上限所在的三個面共有——所以「逐行、無截斷的完整報表」這句在那之後為假：被截的記錄在 CLI 也只有概括句。**不加「完整列舉模式」**：那等於替 CLI 開一條繞過求值上限的路，而上限存在的理由正是讀取路徑不能被 store 內容撐爆；概括句說了「另有 N 個未列出」，沒有東西是沉默的；要全部只能讀 YAML——**那個缺口記成 #581**（R24 verify 第 5／17 列：`replace-endnote-and-zotero` §4 要求能力缺口記 issue，不得靠「讀 YAML」帶過；出口的形狀要在本檔兩面一起裁）。`ValidatePerRecordCapCLITests` 走真 binary 釘住這個契約（25 個配對 → 恰 20 則 ＋「另有 5 個配對未列出」）。CLI 與 MCP 的差別自此只剩**面級**截斷有無 |
| `rename` | 有理由缺席 | 高風險身分操作（citekey 遷移含 verdict value 重寫，#232）＝維運例外 |
| `rename-person`（#395） | 有理由缺席 | 同 `rename` 的理由，**而且遷移面更大**：除 verdict value（`person:` holder，掛在 person、organization 與 venue 三處——R21 verify 第 35 列：這裡曾寫「兩處」，#463 起 venue 迴圈也在）外，還含全庫 `authors[].key` 與 divergence 的 `candidates`／`judgement.prefers`。**這一列不是從上一列類推來的**——它的參照集合是照 `entity-backlink-completeness` 的封閉列舉逐條窮舉出來的（第 1、9、10、13 條），與 citekey 的那組**不重疊**。維運例外的理由因此更強而非更弱：漏一格的後果是安靜的（檔案照樣載入，只是某些邊指向不存在的 key） |
...
| `--yes`（`LibraryOptions`，對 `DestructiveTargetGate.destructiveCommands` 列的 subcommand 生效——2026-09-16 實測 11 個；R21 verify 第 33 列：這裡曾寫死「6 個」；**#580 裁決**：閘防的是**寫錯 store**，不是寫錯哪幾筆——從錯的 store 列出來的 id 在錯的 store 上全部對得上，所以逐 id 的寫入腿同樣要閘（比照 `enrich`／`enrich-from-zotero` 的既有裁決）。`resolve-venues` 的全部寫入腿與 `authorize-names --apply`（R1 verify：布林全庫掃蕩，因宣告寫成 `: Bool` 漏在稽核外）自 #580 起在表內，`resolve-organizations` 的篩選式 `--reject` 也閘。**表外仍有未閘的寫入命令**（預設就寫的格式化、遷移與匯入一族，`resolve-people` 的逐 id 腿，不可逆的合併……清單在 #653，這裡刻意不逐一點名命令——本檔的稽核把反引號裡的命令名當成裁決列的引用）——逐格裁決記 #653，這裡刻意不寫封閉列舉（#580 R1 的上一版寫了「不閘的只有一類」，當場被找到六個反例）。**本列先前的兩句是錯的**：「`resolve-venues` 的 `--apply` 正是篩選式批次掃蕩」（它收 id 清單）、「`rename-person` 沒有這道閘」（它無條件呼叫）；`rename` 沒有閘而 `rename-person` 有，另案 #650）| 有理由缺席（#298）| **閘門本身不作用於 MCP，所以它的出路也不需要**。`--yes` 是「破壞性寫入未指名目標 store 時的知情同意」的出路（#298 D4；#580 起涵蓋 `resolve-organizations --reject` 與 `resolve-venues` 的各寫入腿），而該閘門刻意只擋 CLI：MCP 的 active store 是 session 狀態、「本次呼叫有沒有顯式指定」恆為否（#310），CLI 則每次呼叫獨立解析——閘防的寫錯 store 只在 CLI 那一面有「每次重新解析」這個形狀。（先前這裡的理由是「CLI 的 `--apply` 是篩選式掃蕩、MCP 收逐 id 清單」——#580 把逐 id 的 `resolve-venues`／`enrich` 也放進閘之後，那個理由不再成立。）若日後 MCP 長出篩選式批次寫入，該閘門與本旗標的 MCP 面須一併重新裁決 |
