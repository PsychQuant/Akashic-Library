...
for g in GUARDS:
    ...
    for line in decl_lines:
        ...
        # `rule-coverage.sh` 的宣告改成 `Sources/*/*.swift`，而它**不讀**
        # `Sources/`——那是**編造的宣告**，測的是「中間萬用字元讓痕跡檢查走回
        # 一個不在原始碼裡的段」（R22e 的性質），不是「宣告為真卻被誤殺」。
        #
        # 論證不受影響：R24f 只需要**一個**真實的誤殺例子就成立（誤殺目標使用者
        # 的代價 vs 零實例的漏放）。但「兩種都有背書」是一句沒被檢查的話，而它
        # 出現在**剛刪掉一個稻草人的同一段**裡。
        #
