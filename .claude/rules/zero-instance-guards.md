...
| 30 | **零實例，而上限守的記錄設計上只增不減**（#619 R1 verify security：未決腿的三個上限——一次 200 個 id、20 個 digest、單句說明 4,096 位元組。2026-09-25 實測 live store 8,747 筆 reference：最長的判定理由 687 位元組、rests-on 最多 3 個；`resolution-undecided` 0 筆——format 19 尚未部署。重跑腳本見表下方） | ✅ **寫（整批拒絕、零寫入、不截斷）** | 第 18 列的上限守的是一次遞進來的字串，並夾在兩個量測錨點之間；這一列多一件事：**被守的記錄設計上不退役**——未決是查證歷史，只收位元組相同的重複，一個會迴圈的呼叫端每跑一次就多留一筆。說明與 digest 的上限各有量測錨點（687 位元組的約 6 倍、3 個的約 7 倍）；**id 數 200 沒有可量的母體**——它限的是一次呼叫的批次大小、不是記錄內容，取 CLI 單批 triage 的量級，這一格是推估不是量測（R2 verify DA 第 16 列）。**誠實邊界**：上限只約束一次呼叫、不約束累積——累積由預算 warning 出聲：venue 側是第 16 列，person 與 organization 側是第 31 列（#645；organization 側由 #643 的未決腿加入）。**觸發條件可檢查**（腳本見表下方）：任一筆 reference 的說明或 rests-on 逼近上限時重開。**#645 落地、原本寫下的第二個觸發條件成立（2026-09-26）：重開後裁決不變**——三個上限仍只擋單次呼叫，累積改由第 31 列出聲（節點與位元組兩軸）；上限的量測錨點沒有變 |
| 31 | **零實例，而它是一條已有守衛的曲線換了持有者**（#645：person／organization 持有的 resolution verdict 數逼近 decode 預算。第 16 列守 venue 側；person 側的增長來源是一個人的著作數，#619 起多了不退役的 `resolution-undecided`，#643 起 organization 同。2026-09-25 實測 live store：person **4,575** 筆、單筆最多 **40**（`yi-hau-chen`）；organization **13** 筆、單筆最多 **1**；門檻 11,111——0 實例。重跑腳本見表下方） | ✅ **寫（warning 級、在 `StoreHealth`，門檻沿用第 16 列）** | 第 16 列的理由是「裁決依賴守衛」——暫不改形狀之所以可接受，是因為漲到門檻時工具會出聲；這一列是**同一條曲線的另一個持有者一直沒有那盞燈**，而讓它開始漲的是一個新面（未決腿）。所以理由不是第 1 列的「不寫就沒有跡象」，是**第 30 列自己寫下的誠實邊界**（上限只擋單次呼叫、不擋累積）要有工具面兌現，否則那句邊界就是一句沒有後續的散文。**兩軸**（#645 verify security）：decode 閘除了節點還有讀取位元組上限（8 MiB），而未決記錄的說明可寫到 4,096 位元組——只看節點時，約 2,000 筆就先撞上位元組上限、那時節點數只到預算的一成；所以另設位元組門檻 `verdictByteWarningThreshold`＝讀取位元組預算的一半 ÷ 第 18 列量到的最壞 YAML 跳脫（4.00×）＝ 1 MiB，venue 族一併套用（第 16 列的同一個盲點）。**門檻不另立常數**：person／organization 的 verdict 與 venue 同一個 `ProvenanceReference` 形狀、每筆同為 `nodesPerVenueVerdict` 個節點、受同一個 decode 硬預算約束——另立一個等值常數是同一件事的第二份描述（`no-compat-fallback` §「同一件事只能有一份描述」）；等效筆數的算法（rests-on 換算）抽成 `verdictBudgetCount` 一份、兩族共用。**前綴分開**（`holderVerdictBudgetPrefix`）：兩族的處置不同——venue 是 O(catalog) 的增長、處置是重開第 13 條邊的規模化裁決；person 先要查的是有沒有呼叫端在重複記未決。**觸發條件可檢查**（指令見表下方）：計數應恆為 0；非零時先看那筆記錄的 `resolution-undecided` 筆數 |
...
**第 31 列的量測（2026-09-26 改寫，可重跑）**：warning 數 `grep -a -q 'person／organization 的 verdict 數逼近' "$(command -v akashic)" && akashic validate 2>&1 | grep -c 'person／organization 的 verdict 數逼近'`（應印 0；沒印任何東西＝舊 binary，同第 13 列的自證）。Python 對照鏡射 `verdictBudgetCount` 的兩軸（等效筆數以 rests-on 換算、內容位元組為 value＋說明＋rests-on 的 UTF-8 總和；初版只數原始筆數，與工具的定義不同——#645 verify Codex），讀不到的檔計數（同第 24 列）：

```bash
python3 - <<'EOF'
import glob, io, os, yaml
FIELDS = {'resolution-confirmed', 'resolution-rejected', 'resolution-undecided'}
def budget(refs):
    n = extra = size = 0
    for r in refs or []:
        if not isinstance(r, dict) or r.get('field') not in FIELDS: continue
        n += 1; ro = r.get('rests-on') or []
        extra += len(ro); size += len(str(r.get('value', '')).encode()) + len(str(r.get('judgement', '')).encode()) + sum(len(str(x).encode()) for x in ro)
    return n + (extra + 8) // 9, size
best = {'person': [0, 0], 'organization': [0, 0]}; n = {'person': 0, 'organization': 0}; bad = 0
for f in glob.glob(os.path.expanduser('~/.akashic/entities') + '/*.yaml'):
    try: d = yaml.safe_load(io.open(f, encoding='utf8'))
    ...
    for k in best:
        if k in d:
            n[k] += 1; e, b = budget(d.get('references'))
            best[k] = [max(best[k][0], e), max(best[k][1], b)]
print(n, '單筆最大 [等效筆數, 內容位元組]', best, '讀不到的檔', bad, '門檻 11111 筆／1048576 位元組')
EOF
# 2026-09-26 重跑見 changelog/2026-09-25-holder-verdict-budget.md 的量測節
```
