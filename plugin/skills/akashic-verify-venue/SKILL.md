...
3. **逐次查詢證據清單**——每次查詢一列（同一源查了兩個端點或兩個號就是兩列）：URL＋取得日期＋支撐哪一段（連到了但沒有一筆是本刊——零命中或回的都是別的刊——寫「查無」；同時命中多本姊妹刊寫「多刊命中：<哪幾本>」；回應明確指向另一本刊（例如 DOI 的 container-title 是別的刊）寫「反證：<一句>」；是本刊但與沿革某一段對不上寫「衝突：<哪一段、各源各說什麼>」，那一段在第 1 項標未定；被擋或交回轉址寫「不可達」；某一源本輪沒查就為它寫一列「未需要」）；第 4 源那一列是使用者回覆的文字與日期、「待看」、「不可達」（請了沒回）或「未需要」
