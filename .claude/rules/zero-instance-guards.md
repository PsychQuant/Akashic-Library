...
2. **欄位**（2026-08-24 由 #394 的 `Organization.ror` 顯式擴入，見第 9 列）：一個模型裡的
   欄位，而當下全庫零筆記錄使用它。
...
| 12 | **零實例、成本高，而它屬於另一個型別**（#365：`Author.pseudonym`。筆名是「這個人也用那個名字發表」——關於**人**的事實，`PersonNames` 的 `variant` 分區已經承載；authorship 側只在「同一人在**不同作品**用不同筆名、且兩個名字都要進各自的參考文獻」時才需要 per-occurrence 的名字選擇，**零實例**，且與 #386 的 per-work judged authorship 是同一個位置） | ❌ **不寫** | 與第 10、11 列同為「不寫」而理由是**第三種：位置錯了**。第 10 列缺用途、第 11 列缺理由，這一列連問題都問錯了型別——把筆名放進 `Author` 會讓「人的名字」這個事實有兩個家（`Person.names` 與每一筆 authorship），而 `entity-backlink-completeness` 的單側儲存正是為了讓那種分岔**寫不出來**。APA7 對筆名的處置（用出版時的名字、必要時方括號補真名）是**呈現層**規則，讀的是 `Person.names`——它已經有兩個分區可用。**觸發條件可檢查**：出現第一筆「同一人在不同作品用不同筆名、且兩者都要出現在各自參考文獻」的實例——那時要的是 per-occurrence 的名字選擇，落點在 #386 那一族，不是 `Author.pseudonym` |
...
一句「成本低就寫」會把這九件事壓成同一件，然後在第十個情形上給出沒人同意的答案。
...
它們壓成同一件事，然後在第 13 個情形上給出沒人同意的答案。

**現在還沒出現過的**：零實例、成本**低**、而裁決是「不寫」的情形。前 9 列的成本都低而全部
「寫」，第 10–12 列成本高而「不寫」——這兩軸目前是完全相關的，而**那很可能只是巧合**。
真出現時要加一列，不要從「成本低就寫」推導。
