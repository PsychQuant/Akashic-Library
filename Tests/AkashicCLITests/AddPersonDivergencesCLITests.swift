...
/// #250：`add-person`／`divergences` 兩個新 CLI 面。
///
/// **獨立成檔的教訓**：第一版把本類命名為 `ParityCLITests` 並以 Write 建檔——
/// **整檔蓋掉了 #219 既有的同名檔（22 條測試）**，pre-push 全綠（被刪的測試
/// 不會失敗），靠合併前後的測試計數差才抓回。建「新」檔前先確認路徑不存在。走真 binary（經
...
final class AddPersonDivergencesCLITests: XCTestCase {
    var root: URL!
    var fakeHome: URL!
