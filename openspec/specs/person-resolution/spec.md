...
- **THEN** neither work is rewritten and no verdict is written for it: explicit identifiers, the author-slot operations and the adjudication accept are refused by name, judge and refute skip that pairing by name while the rest proceed (if the index then cannot be rebuilt, the call reports that failure naming the judged or refuted and the skipped pairings), and the bulk apply excludes and lists such candidates while applying the rest
