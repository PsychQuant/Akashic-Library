...
| **查核本身用錯量測**（#407 R25c 當場）| 為了避免第七次而逐項查核前六次的宣稱時，用 `grep -c '一次都沒出現過'` 回 3，把它讀成「3 格用這個 expect」——而那是**字串在檔案中的出現次數**（含註解），真正用它當 expect 的只有 1 格。實跑那格證明守衛行為正確。**結論沒受影響，但工具選錯了**：`grep -c` 數行，問題問的是格。這一列與下一列的差別是層次——下一列是斷言沒被量測，這一列是**量測本身沒被檢查** |
