...
| **一句話被更正十次，而每次更正都留下新的一句**（#407 R21d–R25i）| **形狀**：一個真的查詢，被用來支撐一個那個查詢沒問的性質——載體是「也就是說」「因此」，或**替查詢結果加上查詢沒問的形容詞**。判準與四個實例見上方第 2 題的對照表。**逐次脈絡刻意不在這裡重述**（那正是本表第 112 行記過的錯）——十次的完整經過在 `git log --all --grep='#407 R2[1-5]' --oneline` 與 `plugin/CHANGELOG.md`。**最值得記的一句**：寫下判準不會讓你免疫，只會讓你下次認得出來——第十次就發生在「逐列現查那張判準表」的驗證步驟裡 |
