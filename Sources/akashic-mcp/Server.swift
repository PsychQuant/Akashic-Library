...
actor AkashicMCPServer {
    ...
    static let tools: [Tool] = [
        ...
        Tool(name: "akashic_resolve_people",
             description: "人物解析：不帶 apply/reject 回 {candidates, candidateTotal, candidateRowsDropped, rejectedRowsDropped, pendingTotal, people, ambiguities, ambiguityTotal, ambiguityRowsDropped, truncated}——candidates 是高信心候選（alias 完全命中、不歧義，可套用）；每列帶 counts{confirmed,rejected,pending}（該列證據類別的三態**計數**——刻意不報比率／機率，pendingTotal 給未處理總量）；已否決配對沉底列出（帶 verdict:\"rejected\"、無 id，不可套用）而非隱藏；ambiguities 是同一 literal 對到 2+ person 的位置，**不可套用、需要人判斷**，每筆帶 personRefs（不透明 ref，非 person key）；區辨欄位在 people[ref] 只送一次（key/names/namesTotal/orcid/openalex/died/currentAffiliation 或 formerAffiliation+formerAffiliationEnd|formerAffiliationAttested，缺席即不出現），以便分辨「兩個同名的人」（各自歸屬）與「同一人兩筆記錄」（該合併）。帶 apply（候選 id 陣列）逐候選套用並在同一動作寫 resolution-confirmed verdict；帶 reject（候選 id 陣列）寫 resolution-rejected verdict 到該 person（entry 不動），之後該配對不再被提名（同 literal 他 entry 照提）。apply 與 reject 都是顯式人為動作，無任何自動 verdict 路徑。上限：兩半各 48 KB 位元組預算，歧義至多 50 筆／每筆至多 20 個 personRefs／people 至多 60 筆／每人至多 2 個異名（超出時給 namesTotal），candidates 至多 50 筆（沉底列共用預算、先被擠掉）。吃不下預算的**整列不印**並計入 *RowsDropped；**任一軸被截**都會讓 truncated=true（*Total 給各自的總數）。絕不自動合併。單筆寫入失敗記入 writeFailed／confirmWriteFailed 並續跑（applied 只列實際套用者）。",
             inputSchema: obj([
