...
3. **查不出來** → 不 apply、不 reject，literal 留著，對查過的每個人選記一筆未決（`--undecided`／`undecided`，#619）（store format ≥ 19；可附 `--rests-on`／`rests_on`：承重頁面先 `store-source` 存檔取 digest，一次呼叫的每一筆共用同一組 digest，證據不同就分次送）——之後 CLI 的篩選式 `--apply` 自動排除它，四態計數也不再把它算成未處理；確認過的逐筆顯式送（#624）；只有查到兩筆以上 person 記錄本身可能是同一人時才 `akashic_record_divergence`，且先在回報裡建議，使用者確認後才記——divergence 記了沒有面刪得掉（#586）
