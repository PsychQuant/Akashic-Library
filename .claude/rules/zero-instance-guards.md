...
> 殘留而被人動手**（實作＝猜一個形狀、拿掉＝關掉記錄面）。**新的只有「撞牆」那一項**（R2 verify 第 14 列）：前三類沒有任何一個有
> 使用者面的後果；「被人動手」的兩個方向各與既有形同形——拿掉＝第 3 類的「被當死重刪掉」（差別只在被刪的是 spec 文字還是一行
> `byShape` 條目），實作＝第 10 列的「形狀取決於一個還不存在的用途」。所以「失敗意義不同」**不是**可拿去推第五類的判準，
...
新增本表四類中的任一個（或對第 4 類做出動不動的裁決）時，**在下表加一列**：情形／裁決／理由。（這句曾寫「新增零實例守衛時」——第 2、3 類擴入時就與它分岔，第 4 類再拉寬一級；R2 verify 第 7 列。）**不得依性質相似類推**——覺得
...
新增下一個本表四類中的東西（含對第 4 類的動不動裁決）= 在這張表加一列。
...
for name in sorted(os.listdir(root)):
    ...
    except Exception: unreadable += 1; continue                 # 壞檔不當掉也不靜默——計數印出來，與 validate 的 quarantine 對帳（R2 verify 第 9 列）
...
print(f"organization {n} 筆｜重複群 {dupes} 組｜含 org 候選的 divergence {org_div} 筆｜帶 parents {with_parents} 筆｜讀不到的檔 {unreadable} 個")
EOF
# 2026-09-11：organization 13 筆｜重複群 0 組｜含 org 候選的 divergence 0 筆｜帶 parents 3 筆
# 2026-09-18 R2 重跑（改寫後的腳本）：同——13／0／0／3
# 2026-09-18 R3 重跑（加第五個數）：13／0／0／3／0。對帳：organization 總數 `grep -l '^organization:' ~/.akashic/entities/*.yaml | wc -l`（13）、
