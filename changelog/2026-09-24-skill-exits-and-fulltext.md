...
MCP 列表帶同一個欄位；以三段 id 點名的 apply 照寫，兩段 legacy id 指到這種候選時會被拒絕（apply 與 reject 走同一條解析路徑，兩者都拒）。重複 citekey 這種損壞狀態下，兩個位置可能共用同一個三段 id，這時也拒絕，不猜是哪一筆。tier 閘擋下時，錯誤訊息會說明另有幾筆淘汰所得不會套用。查過但判不出來的配對，工具仍然看不到（#619）。
