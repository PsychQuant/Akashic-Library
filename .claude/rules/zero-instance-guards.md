...
九次的裁決都是「寫」，但**理由各不相同**，這正是不寫總括判準的原因：
...
**很可能是「不寫」**~~ → **2026-08-28 出現了，而且一次兩個**（第 10、11 列，皆出自 #365）。

**那個預測對了一半**：兩列的裁決確實都是「不寫」，但它預測的理由（成本高）**只涵蓋第 11
列**。第 10 列的理由是**形狀取決於一個還不存在的用途**——成本高只是背景，真正擋住它的是
「現在選一個形狀等於用猜的固定一個介面」。

**兩列的理由不可互換，這是本表存在的理由的再一次實例**：一句「零實例且成本高就不寫」會把
它們壓成同一件事，然後在第 12 個情形上給出沒人同意的答案。

**現在還沒出現過的**：零實例、成本**低**、而裁決是「不寫」的情形。前 9 列的成本都低而全部
「寫」，第 10、11 列成本高而「不寫」——這兩軸目前是完全相關的，而**那很可能只是巧合**。
