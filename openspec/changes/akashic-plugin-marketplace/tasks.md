...
- [ ] 6.4 收尾驗證：使用者層設定中不再啟用舊 id，重啟 Claude Code 後 akashic MCP 工具可用。驗證：使用者層 `settings.json` 無 `akashic-mcp@psychquant-claude-plugins`；重啟後呼叫一個 akashic MCP 工具成功；結果記到 #625。 **部分受阻於 #630（2026-09-24）**：設定檔一項已成立（uninstall 自動移除舊 id，僅剩 `akashic-mcp@akashic`、`akashic-discovery@akashic`）；工具呼叫待 #630 發布新 binary 並由使用者重啟後補驗。
