...
> 而且其中一句是**假的**）：
>
> | | 2026-08-22 | 2026-08-23 早 | 2026-08-23 晚 | **2026-08-23 深夜（現行）** |
> |---|---|---|---|---|
> | 守衛支數與總時 | 11 支 59.4 秒 | 13 支 66.5 秒 | 15 支 77.1 秒 | **19 支 99.6 秒** |
> | 最大一支 | `marker-parity-mutations.py` 52.5 秒（88%） | 57.2 秒（86%） | 57.3 秒（74%） | **69.4 秒（70%）** |
> | 其餘 | 十支 7 秒 | 12 支 9.3 秒 | 14 支 19.8 秒 | **18 支 30.2 秒** |
> | **整個 pre-push** | 「59 秒」 | ≈252 秒 | 端到端實測 337 秒 | **端到端 778 秒（#407 R67f，見下方條件）** |
>
> **最後一欄的兩個數字量在 CPU 競爭下，與前三欄不可直接相比**（#407 R67f）：量測當時
> 有一個 21-agent 的跨模型審查 workflow 在同一台機器上跑。778 秒因此**不是**「pre-push
> 變慢了 2.3 倍」的證據——支數從 15 長到 19 只多了 22.5 秒的守衛時間，其餘要歸給競爭與
> `swift test`。**寫出條件而不是只寫數字**，正是本 issue（#407）要求的那件事；一個沒有
> 條件的 778 秒會被下一個讀者拿去跟 337 秒相減，然後得出一個沒有人量過的結論。
> 乾淨環境的重量待補。
>
