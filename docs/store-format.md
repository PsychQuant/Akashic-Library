...
回去看所方原始分類，那裡只有**分割**（三頁互斥的職稱詞彙表）、沒有序——判準是
集合歸屬，「待人工確認」從 1 變 0。領域常常只有分割；序是問題強加的。
...
- `profile.ranks` 等維度是 `TimelineOf<String>` **是刻意的**，不是待補的型別安全——
  ...
  的序）全部被綁架，且**不可逆**——字串可以事後給序，焊死的序拆不回字串
- 同一判準適用於作者序（貢獻排名 vs 姓氏 vs 原文順序）、期刊分級、來源可信度
  （#66）——store **MUST NOT** 為這些提供內建順序
- 這是 #69「同一個 comparator 不得同時服務等價正規化與序列化順序」的同一條線
  再往外一格：同一個 store 欄位不得同時服務「記錄說了什麼」與「它比誰大」
