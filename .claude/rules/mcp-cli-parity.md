...
| `--yes`（`LibraryOptions`，對 `DestructiveTargetGate.destructiveCommands` 列的 subcommand 生效——2026-09-16 實測 11 個；R21 verify 第 33 列：這裡曾寫死「6 個」；**表的判準是「篩選式寫入」，不是「會寫的命令」**（#580 裁決）：成員是帶布林 `--apply`（收窄條件之外全掃）的命令，`resolve-organizations --reject`（收窄後全寫）同形、自 #580 起也閘；**逐 id 顯式指名的寫入腿不閘**——`resolve-venues` 的全部寫入腿（`--apply` 收 id 清單）、`resolve-people` 的 `--reject`／`--judge` 等，理由同本列：它們與 MCP 面同形。封閉列舉在 `DestructiveTargetGate.destructiveCommands` 的 doc。**本列先前的兩句是錯的**：「`resolve-venues` 的 `--apply` 正是篩選式批次掃蕩」（它收 id 清單）、「`rename-person` 沒有這道閘」（它無條件呼叫）；`rename` 沒有閘而 `rename-person` 有，那個不一致另案 #650）| 有理由缺席（#298）| **閘門本身不作用於 MCP，所以它的出路也不需要**。`--yes` 是「破壞性 `--apply` 未指名目標 store 時的知情同意」的出路（#298 D4），而該閘門刻意只擋 CLI：CLI 的 `--apply` 是**篩選式批次掃蕩**，MCP 的 apply 收**逐 id 顯式指名**的清單——與本表上方 `resolve-people` 列已記錄的 tier 閘同型不對稱。#310 的「顯式性在 MCP 面恆為否」因此不綁：閘門不在那一面。若日後 MCP 長出篩選式批次寫入，該閘門與本旗標的 MCP 面須一併重新裁決 |
