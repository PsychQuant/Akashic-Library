...
final class EntityTests: XCTestCase {
    ...
    func testExactAliasMatchYieldsCandidate() {
        ...
        let candidates = PersonResolver.candidates(entries: entries, people: people, rejected: [])
    ...
    func testNoMatchYieldsNothing() {
        ...
        XCTAssertTrue(PersonResolver.candidates(entries: entries, people: people, rejected: []).isEmpty)
    }
    ...
    func testAmbiguousAliasIsExcluded() {
        ...
        let entries = [entry("a2020b", authors: [.literal("Che Cheng")])]
        XCTAssertTrue(PersonResolver.candidates(entries: entries, people: ambiguousPeople, rejected: []).isEmpty)
    }
    ...
    func testResolveDistinguishesNoMatchUniqueAndAmbiguous() {
        ...
        let r = PersonResolver.resolve(entries: entries, people: ambiguousPeople, rejected: [])
    ...
    func testCandidatesIsExactlyResolveCandidates() {
        ...
        XCTAssertEqual(PersonResolver.candidates(entries: entries, people: ambiguousPeople, rejected: []),
                       PersonResolver.resolve(entries: entries, people: ambiguousPeople, rejected: []).candidates)
    }
    ...
    func testApplyConvertsOnlyListedCandidates() {
        ...
        let candidates = PersonResolver.candidates(entries: [e], people: people, rejected: [])
