...
public struct TemporalValue: Equatable, Comparable {
    ...
    public init(value: String, range: DateRange = DateRange(),
    ...
    public static func < (a: TemporalValue, b: TemporalValue) -> Bool {
...
public struct Timeline: Equatable {
    public var entries: [TemporalValue]

    public init(_ entries: [TemporalValue] = []) { self.entries = entries }

    public static func == (a: Timeline, b: Timeline) -> Bool { a.sorted == b.sorted }

    public var isEmpty: Bool { entries.isEmpty }

    /// 目前生效的值（`end == nil` 的最新一段）。多段同時開放時取 `start` 最晚的——
    /// 那是最近一次變更。
    public var current: TemporalValue? {
        entries.filter(\.range.isOpen).max()
    }

    /// 依時間排序（穩定：同區間時按值排）。
    public var sorted: [TemporalValue] { entries.sorted() }

    /// 重疊的段落。**回報而非拒絕**——重疊在真實資料裡可能是對的（一人同時兼兩個
    /// 行政職），也可能是錯的（爬取重複）。判斷屬於使用端，這裡只給事實。
    public func overlappingPairs() -> [(TemporalValue, TemporalValue)] {
        let s = sorted
        var out: [(TemporalValue, TemporalValue)] = []
        for i in s.indices {
...
public struct PersonProfile: Equatable {
    ...
    public var affiliations: Timeline
    /// 職級（升等時間軸）。
    ...
    public init(affiliations: Timeline = Timeline(), ranks: Timeline = Timeline(),
