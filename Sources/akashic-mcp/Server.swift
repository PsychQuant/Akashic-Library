...
actor AkashicMCPServer {
    ...
    static let tools: [Tool] = [
        ...
        Tool(name: "akashic_resolve_people",
             description: "人物解析：不帶 apply 回 {candidates, ambiguities}——candidates 是高信心候選（alias 完全命中、不歧義，可套用）；ambiguities 是同一 literal 對到 2+ person 的位置，**不可套用、需要人判斷**，每個 key 附 names 與 orcid 以便分辨「兩個同名的人」（各自歸屬）與「同一人兩筆記錄」（該合併）。帶 apply（候選 id 陣列）逐候選套用。絕不自動合併。單筆寫入失敗記入 writeFailed 並續跑（applied 只列實際套用者）。",
