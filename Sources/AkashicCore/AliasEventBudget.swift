...
public enum AliasEventBudget {
    ...
    /// 估計展開後的節點數上限。
    ///
    /// **由真實 corpus 校準，不是憑感覺選的**：536 檔實測的最大檔約 700 節點，
    /// 所以 20,000 留了約 28× 餘裕，而已知的最小 bomb 構造（282 bytes、12 層 fanout 2）
    /// 估到 110,611 —— 兩者差 5.5 個數量級，門檻放在中間任何位置都對。
    ///
    /// PR #42 的教訓是**不要用代理指標**（anchor 數 × alias 數）：那種門檻與真實資料
    /// 的距離無法量測，結果真實 corpus 已經坐在上面。這裡的門檻是**同一個量綱**
    /// （節點數）的直接比較，餘裕看得見。
    public static let maxExpandedNodes = 20_000
