...
| **同一段落裡連鎖六次**（#407 R21d–R25c）| 上一列是單次；這一列是它**連續發生六輪**，而且每一輪都在修上一輪。全部不是程式碼錯誤，全部是**描述超出證據**：「收窄是根治」（實為收窄＋一次性手動遮蔽）→「攤開表讓誤宣告可見」（印出來與正常依賴逐字相同）→「19 行單行 `run:`」（19 是總數、單行 13；`13+6+1=20≠19`）→「`case` 不能綁」（實測 3 格綁得起來）→「兩種都有負控背書」（只有一種）→ 而**那句更正本身**又只寫結論不寫查核（「`rule-coverage.sh` 不讀 `Sources/`」為真，但 diff 裡沒有 `grep -c 'Sources' … → 0`）。**位置最傷**：第五次出現在「剛刪掉一個稻草人」的同一段，第六次出現在「修正第五次」的同一段。六次全部由跨模型審查抓出，而且**都在該段話 commit 之後**（`git log --grep=R21d/R22b/R23e/R24g/R25/R25b` 各自對應一個**更正** commit，而非原始 commit）——也就是說**每一次它都通過了作者當下的檢查才進版控**。上一版這裡寫的是「作者自審每一輪都認為那段話已經夠精確」，那是對**心理狀態**的斷言、無從查核，而它偏偏是這一列的結尾金句（#407 R25e，DA 席指名——第七次） |
