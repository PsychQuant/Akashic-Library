...
1. **CLI**：`akashic references --help` 要列出 `extract` 與 `nominate`，而且輸出的 `contract` 要 ≥ 3（第 1、3 步與寫入前各檢查一次）。子命令在、但 JSON 沒有 `contract` 或小於 3 → CLI 是舊版：舊版上本 skill 的寫入前檢查會**空洞地成立**（舊 `nominate` 不回報候選的 `storeMatches`，空清單看起來就像「沒有重複」），而 3 之前的 warning 語意也與下面的處理規則不同（R4 A4）。這兩種情況都停下，請使用者在 Akashic-Library 跑 `swift build -c release --product akashic`，把 `swift build -c release --show-bin-path` 印出的目錄裡的 `akashic` 放上 PATH（發布管道缺口見 #633）。**不要**改用別的方法切分或比對。
...
`<PDF 路徑>` 寫成字面值之前先驗格式：含 `"`、`$`、反引號、反斜線或換行的，不要插進 shell——停下來請使用者把檔案改名或移到沒有這些字元的路徑（R4 B9）。
...
- 結束碼為 0，但 `refs.json` 沒有 `contract` 或小於 3 → 見「開始前」第 1 點，停。
- 讀 `warnings`，每一則都要處理。warning 裡的位置寫成「第 L 行（PDF 第 P 頁）」：告訴使用者時給 **PDF 頁碼**，那是他對照得到的。行號是 `paper.txt` 的座標：不要給使用者，也**不要拿去讀 `paper.txt`**（見上方「不要直接讀」）。
  ...
  - 「…夾在清單中間（第 k 筆之後）：清單在…接續」：清單沒有被截斷；第 k 筆若有一段落在略過的區域裡，會缺那一段。第 k 筆照常判定，標題或出處看起來缺了就判為「判不了」。「清單第一筆之前」的版本表示標題之後先出現了圖表或結束標題，不需要處理。
  ...
  - 「沒有辨識出年份」：可能是殘留的段落文字。
  - 「略過 N 行…落在第 x 筆」：被略過的是頁碼，與頁首頁尾、版權聲明這類重複行（寫著「段內重複」的，是沒有正文可對照時以重複判定的，可能包括正當重複的續行）。列出的那幾筆若標題或出處看起來缺了一段，判為「判不了」。
...
同樣先看結束碼（非零 → 讀 stderr，多半是某個 `--openalex` 檔不是 OpenAlex 的回應，停），再看 `contract`（沒有或小於 3 → 見「開始前」第 1 點，停）。
...
寫入前，**用當下的 store 重跑一次 `nominate`**（先前那次到現在，store 可能被別的 session 改過），確認輸出的 `contract` ≥ 3，而且**每一筆要寫 `cites` 的條目**——要建檔的與「已在庫只連」的都算（R4 A5：只檢查要建檔的，兩次 nominate 之間多出的攣生會讓「已在庫只連」連錯）：
1. 每一組 DOI（含雙胞胎）的 `inStore` 與 `inStoreConflict` 都是空的；
2. 條目的 `storeMatches` 與要建檔那個候選的 `storeMatches` 都是空的，或其中每一筆都已判定為「不是同一篇」；
3. 沒有「被隔離的檔案」warning。

（第 1、2 項不成立的那一筆，照第 4 步重新判定——可能改成「已在庫只連」或「判不了」——不影響其他筆。原本判為「已在庫只連」的，重跑後 `inStore` 變空、`inStoreConflict` 列出兩筆以上、或 `storeMatches` 多了判為同一篇的記錄，都要重判。）
...
- library（D5）：**一律不自動掛**。用 `akashic_libraries(action: "list")` 讀出種子所屬每個 library 的名稱與描述，在乾跑裡列給使用者，由他逐一指定要掛哪個 library、掛哪幾筆（每次執行問一次）；種子屬於 0 個 → 不掛、不問。使用者指定之後，照 [`source-of-truth-over-consent`](../../rules/source-of-truth-over-consent.md) 核對：
  ...
  4. 描述空白、或看不出是上面哪一類 → **不掛**。把 library 的名稱與描述列進報告，請使用者先說明它屬於哪一類（#642 row 3）；同意本身不構成依據。
...
- **切分是啟發式的**：斷點（頁首、圖表、補充資料或致謝等結束標題）之後是否接續，是用「作者按字母順序接得上、年份括號後接標題」判定的。作者—年份格式以外的清單（例如 `[AB99]` 標籤制書目、年份不帶括號的書目）、條目沒有按作者排序的清單、嚴重的排版會切錯。`warnings` 與「只在 PDF／只在 OpenAlex」兩類就是讓這些誤差被看見的地方。R4 用本機 169 份真實 PDF 量過（只記數字）：切得出的 124 份，條目數低於估計值 80% 的有 3 份，3 份都有明確的 warning——標題之前同頁有條目、清單停在圖表標題、多段（R4 A7 更正：R3 版本寫「都有多段或截斷 warning」，實際只有 1 份有，另 2 份無聲）。估計值是「全文中以 `姓, 名縮寫.` 開頭、3 行內帶 `(年份).` 的行數」，抓不到「清單被切走一段、別處又多算」的情形。
