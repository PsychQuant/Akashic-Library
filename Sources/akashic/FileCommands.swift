...
struct FileAdd: ParsableCommand {
    ...
    func run() throws {
        ...
        guard config.files[key] == nil else {
            throw RuntimeFailure.state("key「\(displaySafeInvisible(key, max: 200))」已存在（→ \(displaySafeInvisible(config.files[key]!, max: 300))）。先 file remove 再重加。")
        }
        ...
        // **in-memory 版**：對已載入的 config 比對，不做第二次讀檔（#121 verify：
        ...
        if let existing = try AkashicConfig.key(forPath: absolute, in: config) {
            throw RuntimeFailure.state("路徑已由 key「\(displaySafeInvisible(existing, max: 300))」註冊（同一實體庫不重複註冊——檔案間互不相通）")
        }
...
struct FileUse: ParsableCommand {
    ...
    func run() throws {
        ...
        guard let path = config.files[key] else {
            ...
            throw RuntimeFailure.state("key「\(displaySafeInvisible(key, max: 200))」未註冊。已註冊：\(known.isEmpty ? "（無）" : displaySafeInvisible(known, max: 400))")
        }
        ...
        guard LibraryStore.isLibraryRoot(target) else {
            throw RuntimeFailure.state("「\(displaySafeInvisible(path, max: 300))」不是 Akashic library（缺 entries/ 目錄）。目錄被移走？file remove 後重加。")
        }
        config.current = key
        try config.write(to: options.configURL)
...
struct FileRemove: ParsableCommand {
    ...
    func run() throws {
        var config = try AkashicConfig.read(from: options.configURL)
        guard config.files[key] != nil else {
            throw RuntimeFailure.state("key「\(displaySafeInvisible(key, max: 200))」未註冊。")
        }
