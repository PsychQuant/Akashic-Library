...
| `resolve-divergence` | 有理由缺席 | in-code 既有裁決（`AkashicService.recordDivergence` doc）：消歧含合併＋全庫改寫＋刪檔，tracked+clean 前提與人工確認屬 CLI／App 互動面。**#553（2026-09-11）重新確認，裁決不變**：該輪讓本命令多收一個 shape（venue），而缺席的理由是**操作的性質**（不可逆、要乾跑逐筆過目、要 git 工作樹乾淨），與它收幾個 shape 無關——多一個 shape 不會讓那三件事對 LLM 消費面變得可承擔。`record-divergence` 那一面在 MCP 表已有對應（`akashic_record_divergence`），本輪的 `byShape` 擴充**兩面同時生效**，沒有新的不對稱。**#558 R2（D86）重新確認，裁決不變、契約有改**：tracked+clean 前提自此涵蓋**會被改寫的檔**（倖存者、被改指的 entry、塌縮／改名的歧異記錄），不只被刪的——「要乾跑逐筆過目、要 git 工作樹乾淨」那三件事因此對 LLM 消費面更不可承擔，缺席的理由變強不變弱 |
