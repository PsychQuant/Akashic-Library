...
func swiftGuards() -> [String] {   // #522：ProtectedInventory 也要用
...
func triggerCoverage(argv: [String]) -> Int32 {
    ...
    //
    // **`PATH_ROOTS` 不再是手維護白名單**（#522 附帶問題 1）。它曾經是，而且**漏過兩次**：
    // `mcpb/` 是這道檢查上線第一次執行才補的；#518 R1 重量時又發現漏著 `AkashicApp/`、
    // `Tools/`、`Vendor/`、`mcps/`、`repos/`、`scripts/` 六個。「一份手維護的白名單會漏」
    // 正是這整支守衛在講的事——把它自己也交給機器算。
    //
    // 來源是 repo 根目錄**實際存在的目錄**：新增一個頂層目錄時白名單自動跟上，而
    // 刪掉一個時它自動消失。排除 `.git`（不是內容）與 `.build`（產物，且巨大）。
    let PATH_ROOTS = repoTopLevelRoots()
