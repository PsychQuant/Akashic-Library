...
"split_author": strArray("把一個作者位**拆成多個**（#443）：citekey:authorIndex:分隔符=理由。一個 literal 裝了兩個人時用它。**收分隔符不收拆好的名字**——後者等於讓呼叫端編造；收分隔符則拆出的每一段必然是原文的子字串。切出空段即拒絕；只作用於未歸戶的位置；拆出來的仍是 .literal（拆是形狀修正不是身分判定）。單獨呼叫，不與其餘腿組合——它改的是作者位的數量"),
        ...
        Tool(name: "akashic_resolve_organizations",
             description: "〔#628：作者位候選所在的 work 無法唯一定位（citekey 重複或與另一筆 work 共用 id）時該列帶 unlocatableCitekey:true，apply／reject 這種 id 整批拒絕〕org 解析（#304 parity 移轉）：不帶 apply/reject 回 {candidates, ambiguities}——candidates 是 person affiliations／org parents 的 literal 與某 org name 完全命中且不歧義者。帶 apply（候選 id，形如 holderKey::literal）歸戶並寫 confirmed verdict；帶 reject 寫 rejected verdict。需 store format ≥ 8（verdict）。候選列與歧義條目都帶 id 與 undecidedChecks（候選列是整數；歧義條目是 orgKey → 次數，只列非零），頂層 undecidedTotal（change org-undecided-leg，#643）。",
             inputSchema: obj([
                "apply": strArray("要套用的候選 id（holderKey::literal）；省略＝只列候選"),
                "reject": strArray("要否決的候選 id（同形）"),
                "undecided": strArray("記下查過未決（change org-undecided-leg，#643）：<列表的 id>@<orgKey>=查了什麼、為何判不出來。id 逐字取自這次不帶參數列出的候選列或歧義條目（work 作者位是 citekey[i]::literal）；解析在每個 @<orgKey>= 的位置試切，前綴必須恰為已知的 id——恰一個位置成立才收，零個或多個都整批拒絕（literal 或說明含 @、= 不會切錯）。orgKey 必須是那一列提名的 org（歧義條目可逐個 org 記）。寫一筆 resolution-undecided 到該 org，holder 不動。已判定的配對（該 org 已有 confirmed 或 rejected）與 citekey 無法唯一定位的 work 該筆略過並在 skipped 具名；完全相同的記錄已在＝alreadyRecorded。輸入錯（切不出或切出多個、id 重複、說明空白、org 不存在或不屬於那一列、digest 形狀不合、store format < 19、rests_on 沒有伴隨 undecided）整批拒絕零寫入；一次超過 200 個 id、20 個 digest 或單句說明超過 4,096 位元組同樣整批拒絕。單獨呼叫，不與 apply／reject 組合"),
