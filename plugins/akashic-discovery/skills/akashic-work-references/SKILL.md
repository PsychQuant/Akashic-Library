...
- 結束碼為 0，但 `refs.json` 的 `contract` 不是 4 → 見「開始前」第 1 點，停。
- 讀 `warnings`，每一則都要處理。warning 裡的位置寫成「第 L 行（PDF 第 P 頁）」：告訴使用者時給 **PDF 頁碼**，那是他對照得到的。行號是 `paper.txt` 的座標：不要給使用者，也**不要拿去讀 `paper.txt`**（見上方「不要直接讀」）。
  ...
  - 「清單停在…，而最後一筆（第 k 筆）在那之前還沒結束」：清單在結束標題或圖表標題處停下，而最後一筆在那裡還沒結束——它在斷點之後的那一段不會回來。第 k 筆照常判定，標題或出處看起來缺了一段就判為「判不了」；把 PDF 頁碼告訴使用者（R6）。
  - 「沒有辨識出年份」：可能是殘留的段落文字。
...
同樣先看結束碼（非零 → 讀 stderr，多半是某個 `--openalex` 檔不是 OpenAlex 的回應，停），再看 `contract`（不是 4 → 見「開始前」第 1 點，停）。
...
候選的 `inStore`、或條目與候選的 `storeMatches` 裡出現**種子自己的 citekey** → 這一筆判為「判不了」：一篇論文不會引用它自己，這通常是清單之後的「引用本文」被併進了這一筆（R6）。
...
寫入前，**用當下的 store 重跑一次 `nominate`**（先前那次到現在，store 可能被別的 session 改過），輸出寫到**另一個檔** `<W>/nominations-recheck.json`——第一份 `nominations.json` 要留著，下面第 3 個條件的「新出現」是拿這兩份比出來的（R6）。輸出的 `contract` 要是 4，而且沒有「被隔離的檔案」warning。然後逐筆檢查**每一筆要寫 `cites` 的條目**，依它的類別用不同的條件（R5 D1：R4 把兩類套同一組條件，「已在庫只連」的 `inStore` 依定義不為空，永遠過不了）：
...
- **接受的啟發式模糊**（verify 列出、判定不修，寫在這裡讓讀的人知道）：
  ...
  - 年份之後字詞多於數字的表格列（`(2003) United States 120`）仍可能被當成條目；數字多於字詞的（`(2003) USA, 120, .35`）R6 起不算；
  - 只有清單、沒有正文的輸入，會把清單裡正當重複的行當成頁首刪掉（warning 寫「段內重複」）；
  - 頁首之後第一行認不出是條目開頭時，會併進前一筆的 `raw`；
  - 某一筆的標題換行換出**大寫**、單獨一行的 `Index` 或 `Appendix …`，分不出是標題還是換行，照清單的終點處理（本機語料裡這種行幾乎都是真標題）。那一筆若是最後一筆、而在那之前還沒結束，有「最後一筆還沒結束」warning；它若恰好在句點處換行，沒有 warning（R6）；
  - 吸進清單之後文字的條目，不到 1,000 字時沒有 warning——這一筆自己沒有 DOI、吸進來的文字裡有一個時，`doi` 會是別人的。第 4 步「標題實質相同」的要求擋得住多數這種情形（R6）；
  - 主清單因第一筆之前的圖表而沒被認出、而它的條目開頭不到 3 個、也不多於選中的那段時，沒有 warning（R6 D7：門檻再低，本機語料的目錄與正文句子會誤報）。
