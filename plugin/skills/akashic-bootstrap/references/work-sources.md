...
有 DOI？
  ...
  └─ 無 → 先用標題＋期刊＋年份查出 DOI（見下方「用標題查 DOI」），
           查到後回到上一行
           ⚠ 但**不要用 OpenAlex 的關鍵字搜尋**當這一步的工具——見下
```

### 這一步為什麼不能用 OpenAlex 的關鍵字搜尋

**OpenAlex 的關鍵字／標題搜尋會回傳主題相近的新論文，而不是你要的那一筆。** 相似度排序偏好近期、高被引、metadata 豐富的記錄，而不是你正在找的那筆。

這個失效**不分年代**——使用者 2026-08-20 裁定「不限」：不以出版年份、有無原生 DOI 或被引量設閘，這條路徑對所有缺 DOI 的 work 一律適用。

**改用反向：由你（或 AI）提出候選 DOI，讓 API 判定對錯。**

```
查得到且標題／期刊／年份／type 都對  → 驗證通過
查不到，或任一項對不上              → 標 TODO，不猜
```

候選 DOI 從哪裡來？**從既有的「用標題查 DOI」一族**——Crossref 的 `query.bibliographic`、Europe PMC 的 `TITLE:`。**不得**用 OpenAlex 的關鍵字搜尋產生候選，否則整條路徑循環回本節要避開的那個失效面。
...
**共同結構**：提出候選 → 用識別碼**反向**取回官方記錄 → 以合取判準比對 → 對得上是驗證，對不上或查不到就標 TODO。六種識別碼的型別與正規化見 Akashic repo 的 `Sources/AkashicCore/Identifier.swift`（#394；**該 repo 為 private，無存取權者取不到**）。

**但識別碼終結的是「指涉」不是「描述」**：反向查回來的官方記錄本身可能有誤植（本檔自己的實測，見下方 OpenAlex 一節：出版商把 `Institute of Statistical Science` 寫成複數，**OpenAlex** 原樣保留。上一版把這筆觀察掛到 **Crossref** 名下——而本檔對 Crossref 機構欄位的唯一量測是覆蓋率，不是誤植保留。R8 verify 抓到）。
...
**校正材料的第一順位是同一份檔的其他列。** 更正啟事（erratum）記錄與原記錄共用 DOI，標題結尾是 `(vol X, pg Y, YYYY)`，而它的作者欄**可能補上原記錄缺的東西**。實測型 2 的正確寫法就躺在 23 列之後。

> **但更正啟事列不是校正基準——它只是第二個觀察。**（#389，2026-08-21 實測）
>
> 同一篇論文（DOI `10.2188/jea.je20240034`）在同一份 WoS 匯出裡有原文列與更正啟事列，**兩列各錯一半**：
>
> | | 第 5 位作者 | 第 7 位作者 |
> |---|---|---|
> | 原文列 | `Lai, Tai-Hsuan` ✓ | `Ablm, Yen-Tsung Huang` ✗（ABLM 是證照被當成姓）|
> | 更正啟事列 | `Lai, Tai-Shuan` ✗ | `Huang, Yen-Tsung` ✓ |
>
> 拿任一列去校正另一列都會引入新的錯。**裁決要回到 DOI**——Crossref 對該 DOI 的作者清單一次確定兩者：`Lai, Tai-Hsuan` 與 `Huang, Yen-Tsung`。
>
> 兩列的用處是**互相揭露有東西不對**（同一篇的兩列不一致 ＝ 至少一列錯），不是互相校正。
...
### OpenAlex 有兩個陷阱，而它們在**不同的維度**上（#393）

這一節談的是**機構欄位**的陷阱。本文件開頭還有另一個，兩者容易被讀成同一件事：

| 陷阱 | 在哪一層 | 失效的樣子 | 在本文件的哪裡 |
|---|---|---|---|
| **關鍵字搜尋找錯作品** | **作品查詢層** | 回傳主題相近的**新論文**，而不是你要的那一筆 | 開頭「這一步為什麼不能用 OpenAlex 的關鍵字搜尋」 |
| **`institutions` 是推論** | **機構欄位層** | 找對了作品，但機構欄位被過度歸戶（實測多出一個大學）| 本節 |

**兩者互不蘊含，所以要分開記**：

- 用 DOI 反向 lookup（避開第一個陷阱）**不會**讓第二個消失——你找對了那一筆，它的
  `institutions` 照樣是推論。
- 只取 `raw_affiliation_strings`（避開第二個）**不會**讓第一個消失——你原樣取回的是
  **錯的那篇論文**的機構字串，而那比沒有更糟：它看起來像查證過的資料。

把兩者混為一談的具體後果：以為「我只用 `raw_affiliation_strings`，所以 OpenAlex 這條路
是安全的」——那句話對機構層為真、對作品層為假，而合起來的結論是錯的。
